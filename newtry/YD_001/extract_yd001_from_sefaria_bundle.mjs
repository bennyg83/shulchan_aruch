/**
 * Build YD001 block files from Sefaria-shaped per-seif bundles only (`seif-NNN.json`).
 *
 * Source: `Sefaria Pulls/shulchan-arukh/Yoreh_Deah/simanim/NNN/seif-MMM.json`
 *
 * Usage:
 *   node extract_yd001_from_sefaria_bundle.mjs --siman 1 --from 1 --to 14
 *   npm run bootstrap:yd-simanim -- --from 1 --to 403 --skip-existing
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { yd001OutputSlugFromLayerKey } from "../lib/yoreh_deah_layer_slug.mjs";
import { serializeBlock, EN_PENDING_DEFAULT } from "./yd001_block_lib.mjs";
...
function run() {
  ...
  if (!fs.existsSync(firstBundlePath)) {
    ...
    console.error("Run Phase A rebuild or pass --bundle-root to Yoreh_Deah/simanim");
    process.exit(1);
  }
  ...
  const sourceOrder = layerKeys.map((k) => yd001OutputSlugFromLayerKey(k));
  ...
  for (let seif = from; seif <= to; seif++) {
    ...
    for (const layerKey of layerKeys) {
      const slug = yd001OutputSlugFromLayerKey(layerKey);
  ...
  const manifest = {
    yd001Extract: "by-source",
    source: "Sefaria Pulls — Yoreh_Deah/simanim/NNN/seif-MMM.json (rebuild-by-siman.mjs)",
    dictionaryNote:
