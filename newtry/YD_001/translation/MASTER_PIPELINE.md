# Master pipeline — Yoreh De'ah to completion

**Standing order:** Run **`npm run pipeline:pool:watch`** (daemon) or pool coordinator chat (#5) until `phase: yd_complete`. Target: **4 workers** busy through siman **403** (`finishThrough: 403`).
...
| 5 | `editorial_6_403` | Editorial **105 → 403** (waves of ~96 simanim) |
| 6 | `yd_complete` | Done |
...
```bash
export CURSOR_API_KEY=...   # or YD001_OLLAMA_URL=...
