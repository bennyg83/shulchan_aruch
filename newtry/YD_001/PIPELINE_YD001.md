# YD 001 — Yoreh De'ah (mirror of OC_001)

**Plan:** `YD_YOREH_DEAH_PLAN.md` (repo root).  
...
Canonical Hebrew: `Sefaria Pulls/shulchan-arukh/Yoreh_Deah/simanim/NNN/seif-MMM.json`.
...
From `Sefaria Pulls/shulchan-arukh/Yoreh_Deah`:

```bash
node tools/build-manifest-template-from-yd001.mjs --siman 1 --write-seif 1
node tools/prepare-translated-siman-slice.mjs --siman 1 --from 1 --to 14
```

Or: `npm run publish:prepare:siman1` from `YD_001`.
...
## Step 5 — Publish to `yd1` corpus

```bash
cd "Sefaria Pulls/shulchan-arukh/Yoreh_Deah"
node tools/publish-yd-siman.mjs --siman 1 --skip-rebuild --skip-hebrew
```

Target: `newtry/OC_Mobile/oc318-mobile-reader/public/corpus/yd1/simanN`.
...
| `publish:siman` | One siman → yd1 corpus |
