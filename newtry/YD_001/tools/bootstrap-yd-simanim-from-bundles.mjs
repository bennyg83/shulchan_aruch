#!/usr/bin/env node
/**
 * Extract Yoreh De'ah simanim into `newtry/YD_001/output/siman_NNN/`.
 *
 *   node tools/bootstrap-yd-simanim-from-bundles.mjs --from 1 --to 403 --skip-existing
...
const WORKSPACE = path.resolve(YD001_ROOT, "..", "..");
const EXTRACT = path.join(YD001_ROOT, "extract_yd001_from_sefaria_bundle.mjs");
...
function runExtract(siman, seifTo) {
  ...
  const r = spawnSync(
    ...
    { cwd: YD001_ROOT, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
  );
  if (r.status !== 0) {
...
if (!fs.existsSync(YD_SIMANIM)) {
  ...
  console.error("Run Yoreh_Deah Phase A first (node tools/phase-a.mjs full)");
  process.exit(1);
}

console.log("Bundle root:", YD_SIMANIM);
console.log("YD001 root:", YD001_ROOT);
...
for (let siman = from; siman <= to; siman++) {
  const subdir = `siman_${pad(siman)}`;
  const manifestPath = path.join(YD001_ROOT, "output", subdir, "manifest.json");
