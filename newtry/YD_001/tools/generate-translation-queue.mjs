...
import { COMMENTARY_ORDER, YD001_ROOT } from "../../lib/yd001-volume.mjs";
...
const manifestPath = path.join(YD001_ROOT, "output", `siman_${pad(siman)}`, "manifest.json");
...
const lines = [
  `# YD001 translation queue — siman ${siman}`,
  "",
...
lines.push("", "## Commands", "", "```bash", "cd newtry/YD_001", `npm run apply:dictionary -- --root output/siman_${pad(siman)}`, "npm run pipeline:validate -- --root output/siman_" + pad(siman), "```", "");

const outPath = path.join(YD001_ROOT, "TRANSLATION_QUEUE.md");
