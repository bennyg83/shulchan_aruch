#!/usr/bin/env node
/** Build, patch, dictionary, validate for simanim with _tr-data-NNN.mjs */
...
function run(cmd, args, cwd = ROOT) {
  const r = spawnSync(cmd, args, { cwd, stdio: 'pipe', encoding: 'utf8', shell: process.platform === 'win32' });
...
for (const n of simanim) {
  ...
  if (r.status !== 0) {
    results.push({ n, blocks: 0, status: 'BUILD_FAIL', detail: r.out.slice(-300) });
    continue;
  }
  const mods = fs.readdirSync(WORK).filter((f) => f.startsWith(`_tr-${n}-`) && f.endsWith('.mjs') && !f.includes('data'));
  r = run('node', [`pipeline/work/_patch-siman-${n}-editorial.mjs`, ...mods], ROOT);
  const m = r.out.match(/\[DONE\] (\d+) blocks/);
  const blocks = m ? Number(m[1]) : 0;
  if (r.status !== 0) {
    results.push({ n, blocks, status: 'PATCH_FAIL', detail: r.out.slice(-200) });
    continue;
  }
  ...
  const qOk = r.status === 0;
  r = run('node', ['pipeline/validate-siman-claude-aligned.mjs', '--siman', String(n), '--fail-on', 'error']);
  const pass = qOk && r.status === 0;
  if (pass) {
    fs.appendFileSync(path.join(ROOT, 'progress.log'), `${new Date().toISOString().slice(0, 19)} siman_${String(n).padStart(3, '0')} editorial CLEAN (quality-gate)\n`);
  }
  results.push({ n, blocks, status: pass ? 'PASS' : 'FAIL', detail: pass ? '' : r.out.split('\n').find((l) => l.includes('error') || l.includes('Fail')) || '' });
}

console.log('| siman | blocks | status |');
console.log('|-------|--------|--------|');
for (const { n, blocks, status, detail } of results) {
  console.log(`| ${n} | ${blocks} | ${status}${detail ? ' — ' + detail.slice(0, 50) : ''} |`);
}
