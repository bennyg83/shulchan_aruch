#!/usr/bin/env node
/** Apply editorial translation modules for siman 124 */
import fs from 'fs';
...
if (!modules.length) {
  console.error('Usage: node _patch-siman-124-editorial.mjs _tr-124-baer-heitev.mjs ...');
  process.exit(1);
}
...
function patchFile(rel, slug, T) {
  ...
  const out = parts.map((block, i) => {
    ...
    const key = `${seif}#${marker === 'main' ? '_' : marker}`;
