 ...
 *
 * Backends (YD001_POOL_BACKEND or auto-detect):
...
const SYSTEM = `You are a halachic Hebrew-to-English translator for Shulchan Aruch Yoreh De'ah.
Rules:
...
async function translateWithCursorSdk(prompt) {
  ...
  try {
    ...
    console.error("[worker] Install @cursor/sdk or set YD001_OLLAMA_URL");
    return null;
  }
...
async function translateBlock(item, backend) {
  ...
  const url = process.env.YD001_OLLAMA_URL || "http://127.0.0.1:11434";
  const model =
    process.env.YD001_OLLAMA_MODEL_TRANSLATE ||
    process.env.YD001_OLLAMA_MODEL ||
...
function translateWithClaudeCli(unitId, batchPath, siman, part, parts) {
  ...
  const prompt = [
    `YD001 pool worker — complete unit ${unitId}.`,
...
function detectBackend() {
  const forced = (process.env.YD001_POOL_BACKEND || "").toLowerCase();
  ...
  if (process.env.YD001_OLLAMA_URL || process.env.YD001_POOL_USE_OLLAMA === "1") return "ollama";
  if (process.env.YD001_POOL_USE_CLAUDE_CLI === "1") return "claude-cli";
...
async function main() {
  ...
  if (!backend) {
    console.error(
      "No translation backend. Set YD001_POOL_BACKEND=ollama + YD001_OLLAMA_URL, or claude-cli, or CURSOR_API_KEY."
    );
    process.exit(1);
  }
