...
function loadMaster() {
  ...
  if (!fs.existsSync(p)) {
    return {
      ...
      editorial105: { from: 105, to: 403, skipSimanim: [101, 102, 103, 104] },
...
export function detectPhase() {
  ...
  if (phase === "yd_complete") return { phase, master: m, activeStats: stats };

  if (phase === "editorial_6_403") {
    const finishThrough = m.finishThrough ?? m.editorial105?.to ?? 403;
    ...
    if (planIdle(stats)) return { phase: "yd_complete", master: m, activeStats: stats };
...
export function advanceIfIdle() {
  ...
  if (phase === "ready_editorial_105" || (phase === "html_presentation" && fs.existsSync(HTML_DONE))) {
    ...
    const to = m.editorial105?.to ?? 403;
    const out = m.editorial105?.planFile || "sprint-plan-105-403.json";
    if (!fs.existsSync(path.join(WORK, out))) {
      runPlan(from, to, out);
    }
  ...
  if (phase === "editorial_6_403" && planIdle(stats)) {
    const finishThrough = m.finishThrough ?? 403;
    ...
    m.phase = "yd_complete";
    saveMaster(m);
    return { advanced: true, phase: "yd_complete", action: "Yoreh De'ah editorial complete" };
  }
