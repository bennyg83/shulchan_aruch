 ...
 *
 * Backends: YD001_POOL_BACKEND=ollama | claude-cli | cursor | ide (assign-only)
 */
import fs from "fs";
...
function parseArgs() {
  ...
  if ((process.env.YD001_POOL_BACKEND || "").toLowerCase() === "ide") opts.assignOnly = true;
  return opts;
}
...
async function cycle(opts, cycleNum) {
  ...
  if (!backend) {
    log(
      "STOP: set YD001_POOL_BACKEND=ollama + YD001_OLLAMA_URL, or claude-cli, or --assign-only for IDE workers"
    );
    return false;
  }
  ...
  if (phase === "yd_complete") {
    log("Yoreh De'ah editorial complete (yd_complete).");
    return false;
  }
