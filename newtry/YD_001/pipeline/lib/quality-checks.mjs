...
/** Residual HTML markup in English (should be plain text only). */
const HTML_TAG_IN_EN_RE = /<\s*(?:\/)?(?:b|i|small|br|span|div|p)\b/i;

/** English wrapped like Hebrew source JSON, e.g. ["<b>…"]. */
const JSON_ARRAY_EN_RE = /^\s*\["/;

/** Hebrew stored as JSON string literal from bad import. */
const JSON_ARRAY_HE_RE = /\["|\\"/;

/** Hebrew–Latin script collision (failed partial MT), e.g. וwrote or דradish. */
const HYBRID_SCRIPT_RE = /[\u0590-\u05FF][a-zA-Z]|[a-zA-Z][\u0590-\u05FF]/;
...
export function runBlockQualityChecks(block) {
  ...
  if (HTML_TAG_IN_EN_RE.test(en)) {
    issues.push({
      code: "html_in_english",
      severity: "error",
      message: "HTML tags in English (plain text only)",
    });
  }

  if (JSON_ARRAY_EN_RE.test(en)) {
    issues.push({
      code: "json_wrapped_english",
      severity: "error",
      message: 'English wrapped in JSON array (e.g. ["…"])',
    });
  }

  if (JSON_ARRAY_HE_RE.test(he)) {
    issues.push({
      code: "json_wrapped_hebrew",
      severity: "error",
      message: 'Hebrew wrapped in JSON string literal (e.g. ["…"] or \\" gereshayim)',
    });
  }

  if (HYBRID_SCRIPT_RE.test(enPlain) && countHebrewChars(enPlain) >= 4) {
    issues.push({
      code: "hybrid_mt_garbage",
      severity: "error",
      message: "Hebrew–English hybrid text in English",
    });
  }
