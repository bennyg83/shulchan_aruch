#!/usr/bin/env node
/**
 * Hands-off editorial pool: coordinator watch (#5) + 4 worker loops until yd_complete.
 *
 *   npm run pipeline:pool:autopilot
 *
 * Requires YD001_POOL_BACKEND=ollama (+ YD001_OLLAMA_URL) or claude-cli.
...
function backend() {
  const b = (process.env.YD001_POOL_BACKEND || "").toLowerCase();
  if (b === "ollama" || process.env.YD001_OLLAMA_URL || process.env.YD001_POOL_USE_OLLAMA === "1")
    return "ollama";
  if (b === "claude-cli" || b === "claude" || process.env.YD001_POOL_USE_CLAUDE_CLI === "1")
    return "claude-cli";
...
No translation backend for autopilot.
  ...
  Ollama (recommended for hands-off):
    set YD001_POOL_BACKEND=ollama
    set YD001_OLLAMA_URL=http://10.100.102.14:11434
    set YD001_OLLAMA_MODEL_TRANSLATE=qwen2.5:14b-instruct

  Claude CLI (Pro, session limits apply):
    set YD001_POOL_BACKEND=claude-cli
