...
/** OC001 / YD001: output/siman_NNN/<slug>/part-*.txt; legacy siman 1: output/<slug>/ when siman_001 missing */
function scanOc001Layout(outRoot, opts) {
  const { section, siman } = opts;
  const bundleSection = section === 'OC001' ? 'OC' : section === 'YD001' ? 'YD' : section || 'YD';
  if (section && !['OC', 'OC001', 'YD', 'YD001'].includes(section)) return [];
