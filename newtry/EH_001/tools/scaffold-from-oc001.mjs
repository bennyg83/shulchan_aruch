#!/usr/bin/env node
/**
 * One-time scaffold: copy OC_001 core pipeline + translation docs → EH_001 (YD paths).
 *
...
import { EH001_ROOT, WORKSPACE } from "../../lib/eh001-volume.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OC_ROOT = path.join(WORKSPACE, "newtry", "OC_001");
const OC_PIPELINE = path.join(OC_ROOT, "pipeline");
const YD_PIPELINE = path.join(EH001_ROOT, "pipeline");
const OC_TRANSLATION = path.join(OC_ROOT, "translation");
const YD_TRANSLATION = path.join(EH001_ROOT, "translation");
...
const REPLACEMENTS = [
  ...
  [/oc001/g, "eh001"],
  [/OC_001/g, "EH_001"],
  [/Orach Chayim/g, "Even HaEzer"],
  [/Orach_Chayim/g, "Even_HaEzer"],
  [/oc_complete/g, "eh_complete"],
  [/oc1/g, "eh1"],
  [/validate-oc001/g, "validate-eh001"],
  [/validate-quality-oc001/g, "validate-quality-eh001"],
  [/apply_dictionary_oc001/g, "apply_dictionary_eh001"],
  [/extract_oc001_from_sefaria_bundle/g, "extract_eh001_from_sefaria_bundle"],
  [/wire-mechaber-en-hooks-oc001/g, "wire-mechaber-en-hooks-eh001"],
  [/import-oc001-english-to-seif-en/g, "import-eh001-english-to-seif-en"],
  [/publish-oc-siman/g, "publish-eh-siman"],
  [/publish-mt-batch-to-web/g, "publish-mt-batch-to-web"],
  [/OC001_MT_/g, "EH001_MT_"],
  [/OC001_OLLAMA_/g, "EH001_OLLAMA_"],
  [/OC001_POOL_/g, "EH001_POOL_"],
  [/orch-worker/g, "eh001-worker"],
  [/\b697\b/g, "178"],
  [/\b200\b/g, "100"],
  ...
  [/105 → 697/g, "6 → 178"],
  [/105–697/g, "6–178"],
  [/finishThrough: 697/g, "finishThrough: 178"],
...
function adaptContent(text, destName) {
  ...
  if (destName === "validate-quality-eh001.mjs") {
    s = s.replace(/validate-quality-eh001/g, "validate-quality-eh001");
  }
...
function main() {
  ...
  for (const f of CORE_PIPELINE) {
    ...
    let destName = f;
    if (f === "validate-quality-oc001.mjs") destName = "validate-quality-eh001.mjs";
  ...
  const dockerDest = path.join(EH001_ROOT, "docker", "libretranslate", "docker-compose.yml");
  if (fs.existsSync(dockerSrc)) {
    let yml = fs.readFileSync(dockerSrc, "utf8");
    yml = yml.replace(/oc001-libretranslate/g, "eh001-libretranslate").replace(/OC001/g, "EH001");
    if (!dry) {
  ...
  console.log(`${dry ? "[dry-run] " : ""}Scaffolded ${n} files into EH_001`);
