...
/** OC001 / EH001: output/siman_NNN/<slug>/part-*.txt; legacy siman 1: output/<slug>/ when siman_001 missing */
function scanOc001Layout(outRoot, opts) {
  const { section, siman } = opts;
  const bundleSection = section === 'OC001' ? 'OC' : section === 'EH001' ? 'YD' : section || 'YD';
  if (section && !['OC', 'OC001', 'YD', 'EH001'].includes(section)) return [];
  ...
  for (const entry of simanDirs) {
    ...
    const result = scanSimanDir(path.join(outRoot, entry.name), bundleSection, simanNum);
