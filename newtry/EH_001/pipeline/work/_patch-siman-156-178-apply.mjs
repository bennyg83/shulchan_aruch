#!/usr/bin/env node
/** EH001 FULL REDO simanim 156–178 — all commentaries, all blocks. */
import fs from "fs";
...
import { MECHABER } from "./_patch-siman-156-178-mechaber.mjs";
...
for (const sim of SIMANIM) {
  const files = listTxtFiles(sim);
  if (!files.length) {
    console.log(`siman_${sim}: SKIP (no folder)`);
    continue;
  }
...
const existingSimanim = SIMANIM.filter((s) => perSiman[s] !== undefined);

for (const sim of existingSimanim) {
...
console.log("\n[COMPLETE] Session done — simanim: 156–178");
