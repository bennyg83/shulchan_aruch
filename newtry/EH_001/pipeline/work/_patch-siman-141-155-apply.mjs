#!/usr/bin/env node
/** EH001 FULL REDO simanim 141–155 — all commentaries, all blocks. */
import fs from "fs";
...
import { MECHABER as MECHABER_141 } from "./_patch-siman-141-mechaber.mjs";
import { MECHABER as MECHABER_142_145 } from "./_patch-siman-142-145-mechaber.mjs";
import { MECHABER as MECHABER_146_150 } from "./_patch-siman-146-150-mechaber.mjs";
import { MECHABER as MECHABER_151_155 } from "./_patch-siman-151-155-mechaber.mjs";
import { PITCHEI_TESHUVA } from "./_patch-siman-141-155-pitchei-teshuva.mjs";
...
function blockKey(block) {
  const mk = block.marker === "_" || !block.marker ? "main" : block.marker;
  return `${block.seif}#${mk}`;
}
...
for (const sim of SIMANIM) {
  ...
  for (const fp of listTxtFiles(sim)) {
    ...
    for (const b of blocks) {
      const patchKey = `${b.seif}#${b.marker}`;
      T[patchKey] = translateBlock(b, sim);
      count++;
    }
...
console.log("\n[COMPLETE] Session done — simanim: 141–155");
