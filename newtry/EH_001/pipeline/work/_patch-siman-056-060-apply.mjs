#!/usr/bin/env node
/** EH001 FULL REDO simanim 056–060 — all commentaries, all blocks. */
import fs from "fs";
import path from "path";
import { execSync } from "child_process";
import { parseBlocksInFile } from "../../eh001_block_lib.mjs";
import { patchFile, ROOT, OUT } from "./_patch-siman-utils.mjs";
import { MECHABER } from "./_patch-siman-056-060-mechaber.mjs";
import { COMMENTARIES } from "./_patch-siman-056-060-commentaries.mjs";

const SIMANIM = ["056", "057", "058", "059", "060"];
...
function lookupCommentary(sim, slug, key) {
  const k = `${sim}/${slug}`;
  if (COMMENTARIES[k]?.[key]) return COMMENTARIES[k][key];
  throw new Error(`Missing commentary ${k} ${key}`);
}
...
function translateBlock(block, sim) {
  ...
  return lookupCommentary(sim, block.slug, key);
}
...
for (const sim of SIMANIM) {
  ...
  try {
    execSync(
      `node pipeline/validate-quality-eh001.mjs --root "${OUT}" --siman ${n} --min-severity error --fail-on error`,
...
console.log("\n[COMPLETE] Session done — simanim: 056, 057, 058, 059, 060");
