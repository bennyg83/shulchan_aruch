 ...
 *
 * Backends: EH001_POOL_BACKEND=ollama | claude-cli | cursor | ide (assign-only)
 */
import fs from "fs";
...
function parseArgs() {
  ...
  if ((process.env.EH001_POOL_BACKEND || "").toLowerCase() === "ide") opts.assignOnly = true;
  return opts;
}
...
async function cycle(opts, cycleNum) {
  ...
  if (!backend) {
    log(
      "STOP: set EH001_POOL_BACKEND=ollama + EH001_OLLAMA_URL, or claude-cli, or --assign-only for IDE workers"
    );
    return false;
  }

  const { phase, activeStats } = detectPhase();
  if (phase === "eh_complete") {
    log("Even HaEzer editorial complete (eh_complete).");
    return false;
  }
