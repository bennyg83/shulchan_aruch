 ...
 *
 * Env: EH001_OLLAMA_URL, EH001_OLLAMA_MODEL
 */
import fs from "fs";
...
import { parseBlocksInFile, serializeBlock, blockKey } from "../eh001_block_lib.mjs";
...
const DEFAULT_URL = process.env.EH001_OLLAMA_URL || "http://10.100.102.14:11434";
const DEFAULT_MODEL = process.env.EH001_OLLAMA_MODEL || "qwen2.5:14b-instruct";
...
async function main() {
  ...
  if (!opts.skipDict && touchedSimanim.size) {
    for (const s of [...touchedSimanim].sort((a, b) => a - b)) {
      ...
      const r = spawnSync(process.execPath, ["apply_dictionary_eh001.mjs", "--root", root], {
        cwd: OC_ROOT,
        stdio: "inherit",
      });
