#!/usr/bin/env node
/**
 * Rebuild worker pool queue from disk: all editorial blocks still open through siman 178.
 *
 *   node pipeline/refresh-open-worker-queue.mjs
 *   node pipeline/refresh-open-worker-queue.mjs --from 21 --to 178 --workers 4
...
function parseArgs() {
  ...
  const opts = {
    ...
    workers: 4,
    assign: false,
    outName: "sprint-plan-21-178-open.json",
  };
