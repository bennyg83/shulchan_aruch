...
function loadMaster() {
  ...
  if (!fs.existsSync(p)) {
    return {
      ...
      editorial105: { from: 105, to: 178, skipSimanim: [101, 102, 103, 104] },
...
export function detectPhase() {
  ...
  if (phase === "eh_complete") return { phase, master: m, activeStats: stats };

  if (phase === "editorial_6_403") {
    const finishThrough = m.finishThrough ?? m.editorial105?.to ?? 178;
    ...
    if (planIdle(stats)) return { phase: "eh_complete", master: m, activeStats: stats };
...
export function advanceIfIdle() {
  ...
  if (phase === "ready_editorial_105" || (phase === "html_presentation" && fs.existsSync(HTML_DONE))) {
    ...
    const to = m.editorial105?.to ?? 178;
    const out = m.editorial105?.planFile || "sprint-plan-105-178.json";
    if (!fs.existsSync(path.join(WORK, out))) {
      runPlan(from, to, out);
    }
  ...
  if (phase === "editorial_6_403" && planIdle(stats)) {
    const finishThrough = m.finishThrough ?? 178;
    ...
    m.phase = "eh_complete";
    saveMaster(m);
    return { advanced: true, phase: "eh_complete", action: "Even HaEzer editorial complete" };
  }
