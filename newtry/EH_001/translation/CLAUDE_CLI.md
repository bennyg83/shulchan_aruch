# Claude Code CLI lane (EH001) — **disabled**
...
**Working directory:** CLI runs with `cwd=newtry/EH_001` so `output/siman_NNN/` is inside the session tree (not `cli_workspace` junction). Uses `--permission-mode acceptEdits` for non-interactive writes.
