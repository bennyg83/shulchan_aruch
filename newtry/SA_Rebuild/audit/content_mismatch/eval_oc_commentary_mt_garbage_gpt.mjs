/**
 * Evaluate OC kit 02 commentary MT-garbage GPT completions (new_en whole-cell).
 *
 *   node eval_oc_commentary_mt_garbage_gpt.mjs
 *   node eval_oc_commentary_mt_garbage_gpt.mjs --parts 1-3
 *   node eval_oc_commentary_mt_garbage_gpt.mjs --incoming <dir>
 *
 * Writes:
 *   02_OC_COMMENTARY_MT_GARBAGE_GPT_KIT_GPT_RESULT.json
 *   02_OC_COMMENTARY_MT_GARBAGE_GPT_KIT_GPT_RESULT_EVAL.json
 *   02_OC_COMMENTARY_MT_GARBAGE_GPT_KIT_GPT_RESULT_EVAL.md
 */
import fs from "fs";
...
const DEFAULT_INCOMING = path.join(AUDIT, "_gpt_incoming_kit02_parts01_03");
...
const CUT_RATIO_STRICT = 0.35;
const MIN_NEW_EN_CHARS = 12;
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--parts") {
      ...
      console.log(
        `Usage: node eval_oc_commentary_mt_garbage_gpt.mjs [--parts 1-3] [--incoming DIR]`
      );
      process.exit(0);
    }
  }
...
function evalCase(kitCase, gptCase) {
  ...
  if (newEn.replace(/\s+/g, "").length < MIN_NEW_EN_CHARS) {
    flags.push("NEW_EN_TOO_SHORT");
  }
...
function main() {
  ...
  const seed = results.find((r) => r.id === SEED_ID);
...
`;
  ...
  if (seed) console.log(`seed 244:5 → ${seed.verdict} (${seed.reason})`);
