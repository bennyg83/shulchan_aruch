/**
 * Apply APPROVE-only OC kit 01 Mechaber cut-EN GPT completions to corpus en.html.
 *
 *   node apply_oc_mechaber_cut_en_gpt.mjs --dry-run
 *   node apply_oc_mechaber_cut_en_gpt.mjs --apply
 *
 * Requires prior eval:
 *   01_OC_MECHABER_CUT_EN_GPT_KIT_GPT_RESULT_EVAL.json
 *   01_OC_MECHABER_CUT_EN_GPT_KIT_GPT_RESULT.json
 *
...
function parseArgs(argv) {
  ...
  for (const a of argv) {
    if (a === "--dry-run") mode = "dry-run";
      ...
      console.log(
        "Usage: node apply_oc_mechaber_cut_en_gpt.mjs --dry-run | --apply"
      );
      process.exit(0);
    }
  }
...
function normalizeEnCell(text) {
  ...
  // Corpus mechaber en.html is plain text with a trailing newline
  if (!t.endsWith("\n")) t += "\n";
  return t;
}
...
function main() {
  ...
  const doc = {
    ...
    seed_244_1: applied.find((a) => a.id === "oc1/siman244/seif-001/mechaber")
      || skipped.find((a) => a.id === "oc1/siman244/seif-001/mechaber")
      || null,
  };
