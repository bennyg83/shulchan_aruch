/**
 * HE_HAS_MORE Likut split ALL — apply APPROVE resegments from GPT eval.
 *
 * Actions: split_en, mixed_resegment_translate — join en_segments with <br /> in en.html only.
 * Hebrew untouched. Skip HOLD/REJECT/SKIP_APPLIED.
 *
 *   node apply_he_has_more_likut_split_all.mjs --dry-run
 *   node apply_he_has_more_likut_split_all.mjs --apply
 */
import fs from "fs";
...
const EVAL = path.join(__dirname, "HE_HAS_MORE_LIKUT_SPLIT_GPT_RESULT_ALL_EVAL.json");
const GPT = path.join(__dirname, "HE_HAS_MORE_LIKUT_SPLIT_GPT_RESULT_ALL.json");
...
function getProposedEn(gptCase) {
  if (gptCase?.en_segments?.length) return gptCase.en_segments;
...
function loadApproveIds(evalDoc) {
  const skipApplied = new Set(
    evalDoc.results
      .filter((r) => r.verdict === "SKIP_APPLIED")
      .map((r) => r.id)
  );
  ...
  if (overrideIds?.length) {
    return overrideIds.filter((id) => {
      if (skipApplied.has(id)) {
        console.warn(`SKIP ${id}: SKIP_APPLIED in eval`);
        return false;
      }
...
function main() {
  ...
  console.log(
    `[he-has-more-likut-split-all] mode=${apply ? "APPLY" : "DRY-RUN"} approved=${approvedIds.length}`
  );
  ...
  const auditPath = path.join(__dirname, "HE_HAS_MORE_LIKUT_SPLIT_ALL_APPLY.json");
  fs.writeFileSync(
    auditPath,
    JSON.stringify(
      {
        ...
        source: "HE_HAS_MORE_LIKUT_SPLIT_GPT_RESULT_ALL",
        approvedIds,
        applied,
        skipped,
        failed,
