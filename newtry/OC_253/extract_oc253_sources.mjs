/**
 * Build per-commentary translation files from oc253 source.html.
 *
 * Order:
 *   1) Mechaber + Rama — single source folder `mechaber/` (main column `.mg-main-shulchanarukh`).
 *   2) Every other parshan column — first-appearance order in the HTML (top-to-bottom seifim,
 *      left-to-right columns within each verse).
 *
 * Large sources are split across part-NNN.txt files (--max-chars).
 *
 * Does not translate: English blocks are placeholders until edited.
 *
 * Usage:
 *   node extract_oc253_sources.mjs [html-path]
 *   node extract_oc253_sources.mjs --out output --max-chars 45000
 */
import fs from "fs";
...
import { serializeBlock, EN_PENDING_DEFAULT } from "./oc253_block_lib.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DEFAULT_HTML = path.join(__dirname, "oc253 source.html");
...
/**
 * @param {string[]} blocks Flat list of serializeBlock outputs in final emission order
 */
...
function run() {
  ...
  /** First time each parshan slug appears in the page (top seif, then next seif, column order). */
  ...
  const manifest = {
    oc253Extract: "by-source",
    dictionaryNote:
      "In-house glossary only: ../../full_dictionary (1).md (apply_dictionary_oc253.mjs — no overrides)",
