/**
 * Replace remaining EN placeholders (iw→en) for CM001 part-*.txt trees.
 *
 ...
 *
 *   node tools/translate-CM001-pending-mymemory.mjs --root output/siman_308 --backend libre --ms 100 --workers 3 --chunk-len 400
 *   node tools/translate-CM001-pending-mymemory.mjs --root output --backend google --ms 4000
 *   node tools/translate-CM001-pending-mymemory.mjs --backend auto --chain google,mymemory,lingva
 *
 * Env:
 *   CM001_MT_CHAIN=google,mymemory,lingva,libre
 *   CM001_MT_MS=100  CM001_MT_WORKERS=3  CM001_MT_CHUNK_LEN=400  (libre defaults)
...
const CM001_ROOT = path.resolve(__dirname, "..");
...
function defaultChain() {
  const fromEnv = process.env.CM001_MT_CHAIN?.trim();
  if (fromEnv) {
    return fromEnv
      .split(",")
  ...
  if (process.env.LIBRE_URL?.trim() || process.env.CM001_MT_INCLUDE_LIBRE === "1") {
    chain.push("libre");
  }
  return chain;
}
...
function defaultMs(backend) {
  const fromEnv = Number(process.env.CM001_MT_MS);
...
function defaultWorkers(backend) {
  const fromEnv = Number(process.env.CM001_MT_WORKERS);
...
function defaultChunkLen(backend) {
  const fromEnv = Number(process.env.CM001_MT_CHUNK_LEN);
...
function parseArgs(argv) {
  ...
  if (!rootRel) throw new Error("Required: --root <folder under CM_001 or absolute>");
  const root = path.isAbsolute(rootRel) ? rootRel : path.join(CM001_ROOT, rootRel);
  const resolvedChain = chain?.length ? chain : defaultChain();
  if (backend === "auto" && !resolvedChain.length) {
    throw new Error("auto backend needs a non-empty --chain or CM001_MT_CHAIN");
  }
...
if (backend === "auto") {
  console.log("Tip: set CM001_MT_CHAIN=google,mymemory,lingva,libre and LIBRE_URL for self-hosted LibreTranslate.");
}
