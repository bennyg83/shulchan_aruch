...
import { COMMENTARY_ORDER, CM001_ROOT } from "../../lib/cm001-volume.mjs";
...
const manifestPath = path.join(CM001_ROOT, "output", `siman_${pad(siman)}`, "manifest.json");
...
const lines = [
  `# CM001 translation queue — siman ${siman}`,
  "",
...
lines.push("", "## Commands", "", "```bash", "cd newtry/CM_001", `npm run apply:dictionary -- --root output/siman_${pad(siman)}`, "npm run pipeline:validate -- --root output/siman_" + pad(siman), "```", "");

const outPath = path.join(CM001_ROOT, "TRANSLATION_QUEUE.md");
