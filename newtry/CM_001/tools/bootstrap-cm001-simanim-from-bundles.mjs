#!/usr/bin/env node
/**
 * Extract Choshen Mishpat simanim into `newtry/CM_001/output/siman_NNN/`.
 *
 *   node tools/bootstrap-yd-simanim-from-bundles.mjs --from 1 --to 427 --skip-existing
...
const WORKSPACE = path.resolve(CM001_ROOT, "..", "..");
const EXTRACT = path.join(CM001_ROOT, "extract_cm001_from_sefaria_bundle.mjs");
...
function runExtract(siman, seifTo) {
  ...
  const r = spawnSync(
    ...
    { cwd: CM001_ROOT, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
  );
  if (r.status !== 0) {
...
if (!fs.existsSync(YD_SIMANIM)) {
  ...
  console.error("Run Choshen_Mishpat Phase A first (node tools/phase-a.mjs full)");
  process.exit(1);
}

console.log("Bundle root:", YD_SIMANIM);
console.log("CM001 root:", CM001_ROOT);
...
for (let siman = from; siman <= to; siman++) {
  const subdir = `siman_${pad(siman)}`;
  const manifestPath = path.join(CM001_ROOT, "output", subdir, "manifest.json");
