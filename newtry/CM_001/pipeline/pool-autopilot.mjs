#!/usr/bin/env node
/**
 * Hands-off editorial pool: coordinator watch (#5) + 4 worker loops until cm_complete.
 *
 *   npm run pipeline:pool:autopilot
 *
 * Requires CM001_POOL_BACKEND=ollama (+ CM001_OLLAMA_URL) or claude-cli.
...
No translation backend for autopilot.
  ...
  Ollama (recommended for hands-off):
    set CM001_POOL_BACKEND=ollama
    set CM001_OLLAMA_URL=http://10.100.102.14:11434
    set CM001_OLLAMA_MODEL_TRANSLATE=qwen2.5:14b-instruct

  Claude CLI (Pro, session limits apply):
    set CM001_POOL_BACKEND=claude-cli
