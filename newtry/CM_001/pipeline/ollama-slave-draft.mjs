 ...
 *
 * Env: CM001_OLLAMA_URL, CM001_OLLAMA_MODEL
 */
import fs from "fs";
...
import { parseBlocksInFile, serializeBlock, blockKey } from "../cm001_block_lib.mjs";
...
const DEFAULT_URL = process.env.CM001_OLLAMA_URL || "http://10.100.102.14:11434";
const DEFAULT_MODEL = process.env.CM001_OLLAMA_MODEL || "qwen2.5:14b-instruct";
...
- Commentator names: Siftei Kohen, Mishnah Berurah, Taz, Beit Yosef — not anglicized variants.
...
async function ollamaChat(url, model, userContent) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Ollama HTTP ${res.status}: ${err.slice(0, 100)}`);
  }
...
async function main() {
  ...
  if (!opts.skipDict && touchedSimanim.size) {
    for (const s of [...touchedSimanim].sort((a, b) => a - b)) {
      ...
      const r = spawnSync(process.execPath, ["apply_dictionary_cm001.mjs", "--root", root], {
        cwd: OC_ROOT,
        stdio: "inherit",
      });
