 ...
 *
 * Backends: CM001_POOL_BACKEND=ollama | claude-cli | cursor | ide (assign-only)
 */
import fs from "fs";
...
function parseArgs() {
  ...
  if ((process.env.CM001_POOL_BACKEND || "").toLowerCase() === "ide") opts.assignOnly = true;
  return opts;
}
...
function hasBackend(opts) {
  ...
  const b = (process.env.CM001_POOL_BACKEND || "").toLowerCase();
  if (b === "ide" || b === "manual") return "ide";
  if (b === "ollama" || process.env.CM001_OLLAMA_URL || process.env.CM001_POOL_USE_OLLAMA === "1")
    return "ollama";
  if (b === "claude-cli" || b === "claude" || process.env.CM001_POOL_USE_CLAUDE_CLI === "1")
    return "claude-cli";
...
async function cycle(opts, cycleNum) {
  ...
  if (!backend) {
    log(
      "STOP: set CM001_POOL_BACKEND=ollama + CM001_OLLAMA_URL, or claude-cli, or --assign-only for IDE workers"
    );
    return false;
  }

  const { phase, activeStats } = detectPhase();
  if (phase === "cm_complete") {
    log("Choshen Mishpat editorial complete (cm_complete).");
    return false;
  }
