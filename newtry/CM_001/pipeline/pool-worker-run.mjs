 ...
 *
 * Backends (CM001_POOL_BACKEND or auto-detect):
...
import { parseBlocksInFile, serializeBlock } from "../cm001_block_lib.mjs";
...
const SYSTEM = `You are a halachic Hebrew-to-English translator for Shulchan Aruch Choshen Mishpat.
Rules:
...
async function ollamaChat(url, model, userContent) {
  ...
  if (!res.ok) throw new Error(`Ollama HTTP ${res.status}: ${(await res.text()).slice(0, 100)}`);
...
async function translateWithCursorSdk(prompt) {
  ...
  try {
    ...
    console.error("[worker] Install @cursor/sdk or set CM001_OLLAMA_URL");
    return null;
  }
...
async function translateBlock(item, backend) {
  ...
  const url = process.env.CM001_OLLAMA_URL || "http://127.0.0.1:11434";
  const model =
    process.env.CM001_OLLAMA_MODEL_TRANSLATE ||
    process.env.CM001_OLLAMA_MODEL ||
...
function translateWithClaudeCli(unitId, batchPath, siman, part, parts) {
  ...
  const prompt = [
    `CM001 pool worker — complete unit ${unitId}.`,
...
function detectBackend() {
  const forced = (process.env.CM001_POOL_BACKEND || "").toLowerCase();
  ...
  if (process.env.CM001_OLLAMA_URL || process.env.CM001_POOL_USE_OLLAMA === "1") return "ollama";
  if (process.env.CM001_POOL_USE_CLAUDE_CLI === "1") return "claude-cli";
...
async function main() {
  ...
  if (!backend) {
    console.error(
      "No translation backend. Set CM001_POOL_BACKEND=ollama + CM001_OLLAMA_URL, or claude-cli, or CURSOR_API_KEY."
    );
    process.exit(1);
  }
