#!/usr/bin/env node
/**
 * Rebuild worker pool queue from disk: all editorial blocks still open through siman 427.
 *
 *   node pipeline/refresh-open-worker-queue.mjs
 *   node pipeline/refresh-open-worker-queue.mjs --from 21 --to 427 --workers 4
...
function parseArgs() {
  ...
  const opts = {
    ...
    workers: 4,
    assign: false,
    outName: "sprint-plan-21-427-open.json",
  };
