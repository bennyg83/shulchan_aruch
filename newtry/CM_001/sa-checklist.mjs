...
/** OC001 / CM001: output/siman_NNN/<slug>/part-*.txt; legacy siman 1: output/<slug>/ when siman_001 missing */
function scanOc001Layout(outRoot, opts) {
  const { section, siman } = opts;
  const bundleSection = section === 'OC001' ? 'OC' : section === 'CM001' ? 'YD' : section || 'YD';
  if (section && !['OC', 'OC001', 'YD', 'CM001'].includes(section)) return [];
