# YD 001 — Choshen Mishpat (mirror of OC_001)

**Plan:** `CM_CHOSHEN_MISHPAT_PLAN.md` (repo root).  
...
Canonical Hebrew: `Sefaria Pulls/shulchan-arukh/Choshen_Mishpat/simanim/NNN/seif-MMM.json`.
...
From `Sefaria Pulls/shulchan-arukh/Choshen_Mishpat`:

```bash
node tools/build-manifest-template-from-cm001.mjs --siman 1 --write-seif 1
node tools/prepare-translated-siman-slice.mjs --siman 1 --from 1 --to 14
```

Or: `npm run publish:prepare:siman1` from `CM_001`.
...
Target: `newtry/OC_Mobile/oc318-mobile-reader/public/corpus/cm1/simanN`.
...
| `publish:siman` | One siman → cm1 corpus |
