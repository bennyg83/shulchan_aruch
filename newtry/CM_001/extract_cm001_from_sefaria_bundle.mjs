/**
 * Build CM001 block files from Sefaria-shaped per-seif bundles only (`seif-NNN.json`).
 *
 * Source: `Sefaria Pulls/shulchan-arukh/Choshen_Mishpat/simanim/NNN/seif-MMM.json`
 *
 * Usage:
 *   node extract_cm001_from_sefaria_bundle.mjs --siman 1 --from 1 --to 14
 *   npm run bootstrap:cm001-simanim -- --from 1 --to 427 --skip-existing
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { cm001OutputSlugFromLayerKey } from "../lib/choshen_mishpat_layer_slug.mjs";
import { serializeBlock, EN_PENDING_DEFAULT } from "./cm001_block_lib.mjs";
...
function run() {
  ...
  if (!fs.existsSync(firstBundlePath)) {
    ...
    console.error("Run Phase A rebuild or pass --bundle-root to Choshen_Mishpat/simanim");
    process.exit(1);
  }
  ...
  const sourceOrder = layerKeys.map((k) => cm001OutputSlugFromLayerKey(k));
  ...
  for (let seif = from; seif <= to; seif++) {
    ...
    for (const layerKey of layerKeys) {
      const slug = cm001OutputSlugFromLayerKey(layerKey);
  ...
  const manifest = {
    cm001Extract: "by-source",
    source: "Sefaria Pulls — Choshen_Mishpat/simanim/NNN/seif-MMM.json (rebuild-by-siman.mjs)",
    dictionaryNote:
