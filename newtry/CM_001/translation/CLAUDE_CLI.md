# Claude Code CLI lane (CM001) — **disabled**
...
**Working directory:** CLI runs with `cwd=newtry/CM_001` so `output/siman_NNN/` is inside the session tree (not `cli_workspace` junction). Uses `--permission-mode acceptEdits` for non-interactive writes.
