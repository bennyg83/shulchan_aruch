# Master pipeline — Choshen Mishpat to completion

**Standing order:** Run **`npm run pipeline:pool:watch`** (daemon) or pool coordinator chat (#5) until `phase: cm_complete`. Target: **4 workers** busy through siman **427** (`finishThrough: 427`).
...
| 5 | `editorial_6_403` | Editorial **105 → 427** (waves of ~96 simanim) |
| 6 | `cm_complete` | Done |
...
```bash
export CURSOR_API_KEY=...   # or CM001_OLLAMA_URL=...
