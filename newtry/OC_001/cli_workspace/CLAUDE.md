# CLAUDE.md — Shulchan Aruch Translation Project (OC001)
...
See the full rule set in the zip `CLAUDE.md` (R1–R10): completeness, no additions, dictionary halachic terms, commentator names, abbreviations, numbers, `{Rama: ...}` glosses, Aramaic, connectives, plain output only.
...
- Lanes: `local` (Cursor subagents), `cli` (Claude Code CLI, **Pro auth in `~/.claude/`** — no API key). Optional `api` lane is not used in this project.
...
Retranslate if you see: her age, the craft, Lord's Prayer, Saturday, hand recoils, first dish, muktzeh as "allocated", Magen Avraham anglicized, etc. (full list in project `CLAUDE.md` from Claude zip).
