/**
 * Replace remaining EN placeholders (iw→en) for OC001 part-*.txt trees.
 *
...
function defaultChain() {
  const fromEnv = process.env.OC001_MT_CHAIN?.trim();
  if (fromEnv) {
    return fromEnv
      .split(",")
  ...
  if (process.env.LIBRE_URL?.trim() || process.env.OC001_MT_INCLUDE_LIBRE === "1") {
    chain.push("libre");
  }
  return chain;
}
...
function parseArgs(argv) {
  ...
  if (!rootRel) throw new Error("Required: --root <folder under OC_001 or absolute>");
  const root = path.isAbsolute(rootRel) ? rootRel : path.join(OC001_ROOT, rootRel);
...
if (backend === "auto") {
  console.log("Tip: set OC001_MT_CHAIN=google,mymemory,lingva,libre and LIBRE_URL for self-hosted LibreTranslate.");
}
