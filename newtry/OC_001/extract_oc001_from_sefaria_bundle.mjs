/**
 * Build OC001 block files from Sefaria-shaped per-seif bundles only (`seif-NNN.json`).
 *
 * Source of Hebrew + segmentation: `Sefaria Pulls/shulchan-arukh/Orach_Chayim/simanim/NNN/seif-MMM.json`
 * (from `tools/rebuild-by-siman.mjs` over `mechaber/merged.json` + each `commentaries/<work>/merged.json`).
 *
 * Does not read AlHaTorah or other HTML exports.
 *
 * Usage:
 *   node extract_oc001_from_sefaria_bundle.mjs --siman 1 --from 1 --to 9
 *   node extract_oc001_from_sefaria_bundle.mjs --siman 2 --from 1 --to 6 --out-subdir siman_002
 *   node extract_oc001_from_sefaria_bundle.mjs --bundle-root "C:/path/to/Orach_Chayim/simanim"
 *
 * Bulk simanim 21+ (skip folders that already have a manifest):
 *   npm run bootstrap:oc-simanim
 *   (see `tools/bootstrap-oc-simanim-from-bundles.mjs`)
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { oc001OutputSlugFromLayerKey } from "../lib/orach_chayim_layer_slug.mjs";
...
function parseArgs(argv) {
  ...
  let to = 9;
  let outDir = "output";
  /** e.g. `siman_002` → writes under `output/siman_002/` so siman 1 flat `output/` stays intact */
  let outSubdir = "";
  let maxChars = 45000;
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (argv[i] === "--out-subdir" && argv[i + 1]) outSubdir = String(argv[++i]).replace(/\\/g, "/").replace(/^\/+|\/+$/g, "");
...
// Hebrew א–ת (22 letters), escaped so the file stays ASCII-clean for parsers
...
function run() {
  const { siman, from, to, outDir: outRel, outSubdir, maxChars, bundleRoot } = parseArgs(process.argv.slice(2));
  ...
  if (!fs.existsSync(firstBundlePath)) {
    ...
    console.error("Run from repo root or pass --bundle-root to Orach_Chayim/simanim");
    process.exit(1);
  }
  ...
  const sourceOrder = layerKeys.map((k) => oc001OutputSlugFromLayerKey(k));
  ...
  for (let seif = from; seif <= to; seif++) {
    ...
    for (const layerKey of layerKeys) {
      const slug = oc001OutputSlugFromLayerKey(layerKey);
  ...
  const baseOut = outSubdir ? path.join(__dirname, outRel, outSubdir) : path.join(__dirname, outRel);
  ensureDir(baseOut);
  ...
  const manifest = {
    oc001Extract: "by-source",
    source: "Sefaria Pulls — simanim/NNN/seif-MMM.json (rebuild-by-siman.mjs)",
    dictionaryNote:
