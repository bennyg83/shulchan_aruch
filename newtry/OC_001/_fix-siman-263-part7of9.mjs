...
import { TRANSLATIONS } from "./_siman263-part7of9-translations.mjs";
...
const FILE_BY_SLUG = {
  "magen-avraham": "output/siman_263/magen-avraham/part-001.txt",
  mechaber: "output/siman_263/mechaber/part-001.txt",
...
console.log(`ok siman 263 part7of9 — ${patchCount} blocks`);

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OC_ROOT = __dirname;
const queuePath = path.join(
  OC_ROOT,
  "pipeline/work/editorial-queue-siman-263-part7of9.json",
);
...
if (!queue.items?.length) {
  ...
  const size = Math.ceil(all.length / 9);
  const start = (7 - 1) * size;
  ...
  console.log(`Rebuilt queue with ${items.length} items from full siman slice`);
}
...
for (const it of queue.items || []) {
  ...
  if (!he) continue;
  if (!en || en.length < 8) {
    console.error("FAIL", it.slug, it.seif, it.marker, "empty_english");
    fail++;
    continue;
  }
  for (const p of MT_PATTERNS) {
    if (p.test(en)) {
      console.error("FAIL", it.slug, it.seif, it.marker, `mt:${p}`);
      fail++;
      break;
    }
  }
  ...
  if (sev === "error") {
    console.error("FAIL", it.slug, it.seif, it.marker, issues.map((i) => i.code).join(","));
    fail++;
  }
}
if (fail) {
