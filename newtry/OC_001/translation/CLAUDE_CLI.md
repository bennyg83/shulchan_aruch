# Claude Code CLI lane (OC001) — **disabled**
...
**Working directory:** CLI runs with `cwd=newtry/OC_001` so `output/siman_NNN/` is inside the session tree (not `cli_workspace` junction). Uses `--permission-mode acceptEdits` for non-interactive writes.
