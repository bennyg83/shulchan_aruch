# Master pipeline — Orach Chayim to completion

**Standing order:** Run **`npm run pipeline:pool:watch`** (daemon) or pool coordinator chat (#5) until `phase: oc_complete`. Target: **4 workers** busy through siman **697** (`finishThrough: 697`).
...
| 5 | `editorial_105_697` | Editorial **105 → 697** (waves of ~96 simanim) |
| 6 | `oc_complete` | Done |
...
- **Simanim ≥ 105:** **No auto-publish** (`noPublishFromSiman: 105` in `orchestrator-state.json`). User must ask to update GitHub Pages.

## Automated watch (preferred)

```bash
export CURSOR_API_KEY=...   # or OC001_OLLAMA_URL=...
