...
2. **`newtry/OC_Mobile/oc318-mobile-reader/public/corpus/oc1/`** — `en.html` per source (if present)
...
| Hebrew commentator names in tags | Plain dictionary names (Magen Avraham, Taz, …) |
| Unbalanced `<...>` | Fix or strip |
| Raw `<i data-commentator=...>` | Remove markup; keep note reference in prose if needed: `(Magen Avraham)` |
...
4. For published simanim: re-run `node pipeline/orchestrator.mjs publish --siman N` after OC001 files are clean
