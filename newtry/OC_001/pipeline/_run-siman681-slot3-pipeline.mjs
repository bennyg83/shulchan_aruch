#!/usr/bin/env node
/** Full apply pipeline siman 681 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he681.mjs");
run("_analyze-bad-mt681.mjs");
run("_build-hand681-seed.mjs");
run("_merge-all-hand681-chunks.mjs");
run("_merge-build-fixes-681.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman681-slot3-part${p}.mjs`);
}
