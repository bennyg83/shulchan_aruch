#!/usr/bin/env node
/** Full apply pipeline siman 479 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he479.mjs");
run("_analyze-bad-mt479.mjs");
run("_build-hand479-seed.mjs");
run("_merge-all-hand479-chunks.mjs");
run("_merge-build-fixes-479.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman479-slot3-part${p}.mjs`);
}
