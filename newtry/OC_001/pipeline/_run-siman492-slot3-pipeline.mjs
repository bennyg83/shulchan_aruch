#!/usr/bin/env node
/** Full apply pipeline siman 492 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he492.mjs");
run("_analyze-bad-mt492.mjs");
run("_build-hand492-seed.mjs");
run("_merge-all-hand492-chunks.mjs");
run("_merge-build-fixes-492.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman492-slot3-part${p}.mjs`);
}
