#!/usr/bin/env node
/** worker-slot-10 — sprint simanim 370-377 to COMPLETE */
import fs from "fs";
...
for (const [siman, expected] of SIMANIM) {
  ...
  run("_inject-manual-fixes-slot10.mjs", [String(siman)]);
  ...
  run("_apply-hand-slot10-siman.mjs", [String(siman)]);
...
fs.appendFileSync(LOG, `${slotTs} worker-slot-10 SLOT COMPLETE simanim 370-377\n`, "utf8");
console.log("\n--- Summary ---");
for (const r of results) {
