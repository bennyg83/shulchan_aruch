...
import { t as mech } from "./mech366-en.mjs";
import { t as beer } from "./beer366-en.mjs";
import { t as gra } from "./gra366-en.mjs";
import { t as small } from "./small366-en.mjs";

const allFixes = new Map();
for (const [k, v] of Object.entries(mech)) {
  allFixes.set(`mechaber:${k}`, v);
}
for (const [k, v] of Object.entries(beer)) {
  allFixes.set(`beer-hagolah:${k}`, v);
}
for (const [k, v] of Object.entries(gra)) {
  allFixes.set(`beur-hagra:${k}`, v);
}
...
for (const slug of slugs) {
  const dir = `output/siman_366/${slug}`;
