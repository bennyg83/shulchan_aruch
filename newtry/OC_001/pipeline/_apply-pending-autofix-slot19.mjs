#!/usr/bin/env node
/** Apply autoFix to editorial-pending blocks; write back if quality ok (worker-slot-19) */
import fs from "fs";
...
if (!siman) {
  console.error("usage: node _apply-pending-autofix-slot19.mjs <siman>");
  process.exit(1);
}
...
for (const [file, keys] of byFile) {
  ...
  const out = blocks.map((b) => {
    ...
    if (pf || sev >= SEVERITY.warn) {
      stillBad.push({ file, key, pf, issues: issues.map((i) => i.code) });
      return b;
    }
    applied++;
    return { ...b, en };
...
console.log("siman", siman, "autofix-applied", applied, "still-warn+", stillBad.length, "pending", pending.length);
if (stillBad.length) {
  console.log("STILL_BAD sample:", JSON.stringify(stillBad.slice(0, 5), null, 2));
}
