#!/usr/bin/env node
/** Full apply pipeline siman 691 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he691.mjs");
run("_analyze-bad-mt691.mjs");
run("_build-hand691-seed.mjs");
run("_merge-all-hand691-chunks.mjs");
run("_merge-build-fixes-691.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman691-slot3-part${p}.mjs`);
}
