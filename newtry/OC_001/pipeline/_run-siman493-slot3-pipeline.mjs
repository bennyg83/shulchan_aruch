#!/usr/bin/env node
/** Full apply pipeline siman 493 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he493.mjs");
run("_analyze-bad-mt493.mjs");
run("_build-hand493-seed.mjs");
run("_merge-all-hand493-chunks.mjs");
run("_merge-build-fixes-493.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman493-slot3-part${p}.mjs`);
}
