#!/usr/bin/env node
/** Full apply pipeline siman 496 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he496.mjs");
run("_analyze-bad-mt496.mjs");
run("_build-hand496-seed.mjs");
run("_merge-all-hand496-chunks.mjs");
run("_merge-build-fixes-496.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman496-slot3-part${p}.mjs`);
}
