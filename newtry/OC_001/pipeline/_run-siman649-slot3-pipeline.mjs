#!/usr/bin/env node
/** Full apply pipeline siman 649 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he649.mjs");
run("_analyze-bad-mt649.mjs");
run("_build-hand649-seed.mjs");
run("_merge-all-hand649-chunks.mjs");
run("_merge-build-fixes-649.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman649-slot3-part${p}.mjs`);
}
