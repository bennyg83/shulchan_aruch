#!/usr/bin/env node
/** Merge siman489-part{1,2,3}.json + cite + mechaber → _fixes-siman489-part{1,2,3}.mjs */
import fs from "fs";
...
import { PART1, PART2, PART3 } from "./_analyze-bad-mt489.mjs";
...
async function loadHand() {
  const mech = await import(pathToFileURL(path.join(__dirname, "mech489-en.mjs")).href);
  const hand = {};
  for (const [k, en] of Object.entries(mech.t || {})) hand[`mechaber/${k}`] = en;
  for (const f of ["siman489-part1.json", "siman489-part2.json", "siman489-part3.json"]) {
...
function listPartFiles(slug) {
  const dir = path.join(ROOT, `output/siman_489/${slug}`);
  if (!fs.existsSync(dir)) return [];
  return fs
    .readdirSync(dir)
    .filter((f) => f.endsWith(".txt"))
    .sort()
    .map((f) => `output/siman_489/${slug}/${f}`);
}
...
function buildPart(slugs, partNum, hand) {
  ...
  const outPath = path.join(__dirname, `_fixes-siman489-part${partNum}.mjs`);
  fs.writeFileSync(
    outPath,
    `/** siman 489 part ${partNum} — Pesach */\nexport const fixes = ${JSON.stringify(fixes, null, 2)};\n`
  );
