#!/usr/bin/env node
/** Merge siman455-part{1,2,3}.json + cite + mechaber → _fixes-siman455-part{1,2,3}.mjs */
import fs from "fs";
import path from "path";
import { fileURLToPath, pathToFileURL } from "url";
import { parseBlocksInFile } from "../oc001_block_lib.mjs";
import { translateCite455 as translateCite } from "./lib/translate-cite-455.mjs";
import { isBadMt447 as isBad } from "./lib/bad-mt-447.mjs";
import { PART1, PART2, PART3 } from "./_analyze-bad-mt455.mjs";
...
async function loadHand() {
  const mech = await import(pathToFileURL(path.join(__dirname, "mech455-en.mjs")).href);
  const hand = {};
  for (const [k, en] of Object.entries(mech.t || {})) hand[`mechaber/${k}`] = en;
  for (const f of ["siman455-part1.json", "siman455-part2.json", "siman455-part3.json"]) {
...
function listPartFiles(slug) {
  const dir = path.join(ROOT, `output/siman_455/${slug}`);
  if (!fs.existsSync(dir)) return [];
  return fs
    .readdirSync(dir)
    .filter((f) => f.endsWith(".txt"))
    .sort()
    .map((f) => `output/siman_455/${slug}/${f}`);
}
...
function buildPart(slugs, partNum, hand) {
  ...
  const outPath = path.join(__dirname, `_fixes-siman455-part${partNum}.mjs`);
  fs.writeFileSync(
    outPath,
    `/** siman 455 part ${partNum} — mayim shelanu */\nexport const fixes = ${JSON.stringify(fixes, null, 2)};\n`
  );
