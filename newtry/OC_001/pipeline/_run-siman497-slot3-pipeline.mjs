#!/usr/bin/env node
/** Full apply pipeline siman 497 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he497.mjs");
run("_analyze-bad-mt497.mjs");
run("_build-hand497-seed.mjs");
run("_merge-all-hand497-chunks.mjs");
run("_merge-build-fixes-497.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman497-slot3-part${p}.mjs`);
}
