#!/usr/bin/env node
/** Full apply pipeline siman 673 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he673.mjs");
run("_analyze-bad-mt673.mjs");
run("_build-hand673-seed.mjs");
run("_merge-all-hand673-chunks.mjs");
run("_merge-build-fixes-673.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman673-slot3-part${p}.mjs`);
}
