#!/usr/bin/env node
/** Full apply pipeline siman 676 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he676.mjs");
run("_analyze-bad-mt676.mjs");
run("_build-hand676-seed.mjs");
run("_merge-all-hand676-chunks.mjs");
run("_merge-build-fixes-676.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman676-slot3-part${p}.mjs`);
}
