#!/usr/bin/env node
/** Full apply pipeline siman 499 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he499.mjs");
run("_analyze-bad-mt499.mjs");
run("_build-hand499-seed.mjs");
run("_merge-all-hand499-chunks.mjs");
run("_merge-build-fixes-499.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman499-slot3-part${p}.mjs`);
}
