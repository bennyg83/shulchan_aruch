/** Shared worker-slot-5 editorial helpers */
import fs from "fs";
...
export function writeApplyScript(pipelineDir, siman, batchNum) {
  const applyPath = path.join(pipelineDir, `_apply-siman${siman}-batch${batchNum}-slot5.mjs`);
  const content = `#!/usr/bin/env node
/** worker-slot-5 — siman ${siman} editorial batch ${batchNum} */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { parseBlocksInFile, serializeBlock } from "../oc001_block_lib.mjs";
import { FIXES } from "./_siman${siman}-slot5-batch${batchNum}-data.mjs";
...
const sync = spawnSync(
  process.execPath,
  [path.join(__dirname, "sync-queue-from-output.mjs"), path.join(__dirname, "work", "editorial-queue-siman-${siman}.json")],
...
export function writeBatchData(pipelineDir, siman, batchNum, batchFixes, count) {
  const outPath = path.join(pipelineDir, `_siman${siman}-slot5-batch${batchNum}-data.mjs`);
  const body = `/** worker-slot-5 — siman ${siman} editorial batch ${batchNum} fixes (${count} blocks) */\nexport const FIXES = ${JSON.stringify(batchFixes, null, 2)};\n`;
  fs.writeFileSync(outPath, body, "utf8");
  return outPath;
}

export function loadHandJson(pipelineDir, siman) {
  const p = path.join(pipelineDir, "work", `hand-slot5-siman-${siman}.json`);
