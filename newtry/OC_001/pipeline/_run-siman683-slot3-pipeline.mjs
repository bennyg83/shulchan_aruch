#!/usr/bin/env node
/** Full apply pipeline siman 683 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he683.mjs");
run("_analyze-bad-mt683.mjs");
run("_build-hand683-seed.mjs");
run("_merge-all-hand683-chunks.mjs");
run("_merge-build-fixes-683.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman683-slot3-part${p}.mjs`);
}
