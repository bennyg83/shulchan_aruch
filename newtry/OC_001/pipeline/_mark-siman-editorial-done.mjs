#!/usr/bin/env node
/** Mark every Hebrew block in a siman as editorial-done (after translations applied) */
import fs from "fs";
...
if (!siman) {
  console.error("usage: node _mark-siman-editorial-done.mjs <siman>");
  process.exit(1);
}
...
console.log("marked", ids.length, "block ids for siman", siman);

const done = loadEditorialDoneIds(WORK);
const pending = collectEditorialBlocks(OUT, siman, "all", "warn", done);
console.log("quality-flagged still open:", pending.length);
