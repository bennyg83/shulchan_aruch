#!/usr/bin/env node
/** Full apply pipeline siman 464 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he464.mjs");
run("_analyze-bad-mt464.mjs");
run("_build-hand464-seed.mjs");
run("_merge-all-hand464-chunks.mjs");
run("_merge-build-fixes-464.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman464-slot3-part${p}.mjs`);
}
