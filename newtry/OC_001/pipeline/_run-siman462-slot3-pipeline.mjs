#!/usr/bin/env node
/** Full apply pipeline siman 462 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he462.mjs");
run("_analyze-bad-mt462.mjs");
run("_build-hand462-seed.mjs");
run("_merge-all-hand462-chunks.mjs");
run("_merge-build-fixes-462.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman462-slot3-part${p}.mjs`);
}
