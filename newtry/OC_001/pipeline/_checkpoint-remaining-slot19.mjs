#!/usr/bin/env node
/** Checkpoint pending editorial blocks + validate + finalize for worker-slot-19 */
...
if (!siman) {
  console.error("usage: node _checkpoint-remaining-slot19.mjs <siman>");
  process.exit(1);
}
...
const fin = spawnSync(process.execPath, [path.join(__dirname, "_finalize-siman-slot19.mjs"), String(siman)], {
  cwd: OC_ROOT,
  stdio: "inherit",
});
