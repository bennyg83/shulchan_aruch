#!/usr/bin/env node
/** Full apply pipeline siman 686 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he686.mjs");
run("_analyze-bad-mt686.mjs");
run("_build-hand686-seed.mjs");
run("_merge-all-hand686-chunks.mjs");
run("_merge-build-fixes-686.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman686-slot3-part${p}.mjs`);
}
