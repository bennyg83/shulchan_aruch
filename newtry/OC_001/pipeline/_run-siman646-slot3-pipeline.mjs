#!/usr/bin/env node
/** Full apply pipeline siman 646 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he646.mjs");
run("_analyze-bad-mt646.mjs");
run("_build-hand646-seed.mjs");
run("_merge-all-hand646-chunks.mjs");
run("_merge-build-fixes-646.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman646-slot3-part${p}.mjs`);
}
