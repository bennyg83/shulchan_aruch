#!/usr/bin/env node
/** Full apply pipeline siman 481 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he481.mjs");
run("_analyze-bad-mt481.mjs");
run("_build-hand481-seed.mjs");
run("_merge-all-hand481-chunks.mjs");
run("_merge-build-fixes-481.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman481-slot3-part${p}.mjs`);
}
