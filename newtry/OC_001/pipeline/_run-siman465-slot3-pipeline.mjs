#!/usr/bin/env node
/** Full apply pipeline siman 465 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he465.mjs");
run("_analyze-bad-mt465.mjs");
run("_build-hand465-seed.mjs");
run("_merge-all-hand465-chunks.mjs");
run("_merge-build-fixes-465.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman465-slot3-part${p}.mjs`);
}
