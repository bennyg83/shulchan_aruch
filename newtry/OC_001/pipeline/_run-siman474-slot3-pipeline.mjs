#!/usr/bin/env node
/** Full apply pipeline siman 474 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he474.mjs");
run("_analyze-bad-mt474.mjs");
run("_build-hand474-seed.mjs");
run("_merge-all-hand474-chunks.mjs");
run("_merge-build-fixes-474.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman474-slot3-part${p}.mjs`);
}
