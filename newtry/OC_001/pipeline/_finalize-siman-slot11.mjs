#!/usr/bin/env node
/** Mark siman editorial complete for worker-slot-11 */
import fs from "fs";
...
if (!siman) {
  console.error("usage: node _finalize-siman-slot11.mjs <siman>");
  process.exit(1);
}
...
const line = `${new Date().toISOString().replace(/\.\d{3}Z$/, "Z")} worker-slot-11 siman_${siman} COMPLETE\n`;
