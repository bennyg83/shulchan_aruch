...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he469-export.json"), "utf8"));
...
export const PART3 = ["ateret-zekenim","chatam-sofer","eliyah-rabbah","kaf-hachayyim","shaarei-teshuvah","yad-ephraim","chokhmat-shlomo"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he469-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he469-bad-p${pn}.json`, badKeys[pn].length);
}
