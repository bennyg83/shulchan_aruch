#!/usr/bin/env node
/** Mark siman editorial complete for worker-slot-14 */
import fs from "fs";
...
if (!siman) {
  console.error("usage: node _finalize-siman-slot14.mjs <siman>");
  process.exit(1);
}
...
const line = `${new Date().toISOString().replace(/\.\d{3}Z$/, "Z")} worker-slot-14 siman_${siman} COMPLETE\n`;
