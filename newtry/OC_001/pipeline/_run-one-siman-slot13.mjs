#!/usr/bin/env node
/** Run full slot13 pipeline for one siman */
...
if (!siman) {
  console.error("usage: node _run-one-siman-slot13.mjs <siman>");
  process.exit(1);
}
...
for (let b = 1; b <= 40; b++) {
  ...
  if (fs.existsSync(hb)) run("_inject-hand-en-slot13.mjs", [String(siman), hb]);
}
