#!/usr/bin/env node
/** Full apply pipeline siman 456 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he456.mjs");
run("_analyze-bad-mt456.mjs");
run("_build-hand456-seed.mjs");
run("_merge-all-hand456-chunks.mjs");
run("_merge-build-fixes-456.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman456-slot3-part${p}.mjs`);
}
