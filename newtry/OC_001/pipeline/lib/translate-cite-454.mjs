/** Beer HaGolah citation lines — siman 454 (matzah types). */
import { translateCite434 } from "./translate-cite-434.mjs";

const EXACT454 = new Map([
  ...
  ['רמב"ם פ"ו', "Rambam chapter 6"],
  ["תוס' שם", "Tosafot there"],
  ["רש\"י שם", "Rashi there"],
]);

export function translateCite454(heRaw) {
  let he = heRaw.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  if (EXACT454.has(he)) return EXACT454.get(he);
