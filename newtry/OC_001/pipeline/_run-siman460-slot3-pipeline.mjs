#!/usr/bin/env node
/** Full apply pipeline siman 460 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he460.mjs");
run("_analyze-bad-mt460.mjs");
run("_build-hand460-seed.mjs");
run("_merge-all-hand460-chunks.mjs");
run("_merge-build-fixes-460.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman460-slot3-part${p}.mjs`);
}
