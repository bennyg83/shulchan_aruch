#!/usr/bin/env node
/** Full apply pipeline siman 690 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he690.mjs");
run("_analyze-bad-mt690.mjs");
run("_build-hand690-seed.mjs");
run("_merge-all-hand690-chunks.mjs");
run("_merge-build-fixes-690.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman690-slot3-part${p}.mjs`);
}
