#!/usr/bin/env node
/** Full apply pipeline siman 672 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he672.mjs");
run("_analyze-bad-mt672.mjs");
run("_build-hand672-seed.mjs");
run("_merge-all-hand672-chunks.mjs");
run("_merge-build-fixes-672.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman672-slot3-part${p}.mjs`);
}
