#!/usr/bin/env node
/** Full apply pipeline siman 476 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he476.mjs");
run("_analyze-bad-mt476.mjs");
run("_build-hand476-seed.mjs");
run("_merge-all-hand476-chunks.mjs");
run("_merge-build-fixes-476.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman476-slot3-part${p}.mjs`);
}
