#!/usr/bin/env node
/** Full apply pipeline siman 485 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he485.mjs");
run("_analyze-bad-mt485.mjs");
run("_build-hand485-seed.mjs");
run("_merge-all-hand485-chunks.mjs");
run("_merge-build-fixes-485.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman485-slot3-part${p}.mjs`);
}
