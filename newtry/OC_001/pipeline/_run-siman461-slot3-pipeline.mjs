#!/usr/bin/env node
/** Full apply pipeline siman 461 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he461.mjs");
run("_analyze-bad-mt461.mjs");
run("_build-hand461-seed.mjs");
run("_merge-all-hand461-chunks.mjs");
run("_merge-build-fixes-461.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman461-slot3-part${p}.mjs`);
}
