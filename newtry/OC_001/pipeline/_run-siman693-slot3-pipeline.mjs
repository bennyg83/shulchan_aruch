#!/usr/bin/env node
/** Full apply pipeline siman 693 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he693.mjs");
run("_analyze-bad-mt693.mjs");
run("_build-hand693-seed.mjs");
run("_merge-all-hand693-chunks.mjs");
run("_merge-build-fixes-693.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman693-slot3-part${p}.mjs`);
}
