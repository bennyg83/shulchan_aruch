#!/usr/bin/env node
/**
 * Full slot6 loop for one siman: preflight → build batches → apply all → finalize
 * Usage: node pipeline/_run-slot6-siman-loop.mjs 257
 */
import fs from "fs";
...
if (!siman) throw new Error("Usage: _run-slot6-siman-loop.mjs <siman>");
...
run("_finalize-siman-slot6.mjs", [String(siman)]);
