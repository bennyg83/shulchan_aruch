...
const parts = [1, 2, 3].map((n) =>
  JSON.parse(fs.readFileSync(path.join(__dirname, `siman453-part${n}.json`), "utf8"))
);
...
for (const pn of [1, 2, 3]) {
  ...
  fs.writeFileSync(path.join(__dirname, `he453-queue-p${pn}.json`), JSON.stringify(out, null, 2) + "\n");
  console.log(`he453-queue-p${pn}.json`, Object.keys(out).length);
}
