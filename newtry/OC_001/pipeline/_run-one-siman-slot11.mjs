#!/usr/bin/env node
/** Run full slot11 pipeline for one siman */
...
if (!siman) {
  console.error("usage: node _run-one-siman-slot11.mjs <siman>");
  process.exit(1);
}
...
run("_seed-hand-slot11-partial.mjs", [String(siman)]);
const fixes = path.join(__dirname, `_fixes-siman${siman}-slot11.mjs`);
if (fs.existsSync(fixes)) run("_inject-hand-en-slot11.mjs", [String(siman), fixes]);
for (let b = 1; b <= 40; b++) {
  const hb = path.join(__dirname, `_hand${siman}-b${b}-en.mjs`);
  if (fs.existsSync(hb)) run("_inject-hand-en-slot11.mjs", [String(siman), hb]);
}
run("_force-seed-hand-slot11.mjs", [String(siman)]);
run("_fix-hand-preflight-slot11.mjs", [String(siman)]);
const audit = spawnSync(process.execPath, [path.join(__dirname, "_audit-hand-slot11.mjs"), String(siman)], {
  cwd: OC_ROOT,
  encoding: "utf8",
});
if (audit.status !== 0) {
