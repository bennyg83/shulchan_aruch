#!/usr/bin/env node
/** Complete one siman for worker-slot-18: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot18.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot18.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot18.mjs", [String(siman)]);
...
run("_finalize-siman-slot18.mjs", [String(siman)]);
