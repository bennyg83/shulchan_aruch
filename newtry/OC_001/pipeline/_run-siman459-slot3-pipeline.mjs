#!/usr/bin/env node
/** Full apply pipeline siman 459 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he459.mjs");
run("_analyze-bad-mt459.mjs");
run("_build-hand459-seed.mjs");
run("_merge-all-hand459-chunks.mjs");
run("_merge-build-fixes-459.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman459-slot3-part${p}.mjs`);
}
