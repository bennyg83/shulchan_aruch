#!/usr/bin/env node
/** Full apply pipeline siman 491 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he491.mjs");
run("_analyze-bad-mt491.mjs");
run("_build-hand491-seed.mjs");
run("_merge-all-hand491-chunks.mjs");
run("_merge-build-fixes-491.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman491-slot3-part${p}.mjs`);
}
