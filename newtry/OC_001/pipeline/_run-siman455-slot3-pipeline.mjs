#!/usr/bin/env node
/** Full apply pipeline siman 455 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he455.mjs");
run("_analyze-bad-mt455.mjs");
run("_build-hand455-seed.mjs");
run("_merge-all-hand455-chunks.mjs");
run("_merge-build-fixes-455.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman455-slot3-part${p}.mjs`);
}
