#!/usr/bin/env node
/** Full apply pipeline siman 639 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he639.mjs");
run("_analyze-bad-mt639.mjs");
run("_build-hand639-seed.mjs");
run("_merge-all-hand639-chunks.mjs");
run("_merge-build-fixes-639.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman639-slot3-part${p}.mjs`);
}
