#!/usr/bin/env node
/** Full apply pipeline siman 457 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he457.mjs");
run("_analyze-bad-mt457.mjs");
run("_build-hand457-seed.mjs");
run("_merge-all-hand457-chunks.mjs");
run("_merge-build-fixes-457.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman457-slot3-part${p}.mjs`);
}
