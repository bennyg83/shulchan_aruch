#!/usr/bin/env node
/** Full apply pipeline siman 680 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he680.mjs");
run("_analyze-bad-mt680.mjs");
run("_build-hand680-seed.mjs");
run("_merge-all-hand680-chunks.mjs");
run("_merge-build-fixes-680.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman680-slot3-part${p}.mjs`);
}
