#!/usr/bin/env node
/** Full apply pipeline siman 687 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he687.mjs");
run("_analyze-bad-mt687.mjs");
run("_build-hand687-seed.mjs");
run("_merge-all-hand687-chunks.mjs");
run("_merge-build-fixes-687.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman687-slot3-part${p}.mjs`);
}
