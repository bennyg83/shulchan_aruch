#!/usr/bin/env node
/** Merge siman691-part{1,2,3}.json + cite + mechaber → _fixes-siman691-part{1,2,3}.mjs */
import fs from "fs";
...
import { PART1, PART2, PART3 } from "./_analyze-bad-mt691.mjs";
...
async function loadHand() {
  ...
  for (const f of ["siman691-part1.json", "siman691-part2.json", "siman691-part3.json"]) {
...
function listPartFiles(slug) {
  const dir = path.join(ROOT, `output/siman_691/${slug}`);
  if (!fs.existsSync(dir)) return [];
  return fs
    .readdirSync(dir)
    .filter((f) => f.endsWith(".txt"))
    .sort()
    .map((f) => `output/siman_691/${slug}/${f}`);
}
...
function buildPart(slugs, partNum, hand) {
  ...
  const outPath = path.join(__dirname, `_fixes-siman691-part${partNum}.mjs`);
  fs.writeFileSync(
    outPath,
    `/** siman 691 part ${partNum} — Pesach */\nexport const fixes = ${JSON.stringify(fixes, null, 2)};\n`
  );
