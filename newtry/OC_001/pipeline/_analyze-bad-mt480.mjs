...
export const PART3 = ["chatam-sofer","eliyah-rabbah","kaf-hachayyim","rabbi-akiva-eiger"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he480-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he480-bad-p${pn}.json`, badKeys[pn].length);
}
