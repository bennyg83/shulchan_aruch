#!/usr/bin/env node
/** Full apply pipeline siman 695 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he695.mjs");
run("_analyze-bad-mt695.mjs");
run("_build-hand695-seed.mjs");
run("_merge-all-hand695-chunks.mjs");
run("_merge-build-fixes-695.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman695-slot3-part${p}.mjs`);
}
