...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he458-export.json"), "utf8"));
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he458-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he458-bad-p${pn}.json`, badKeys[pn].length);
}
