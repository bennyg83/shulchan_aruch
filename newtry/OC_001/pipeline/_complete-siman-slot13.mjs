#!/usr/bin/env node
/** Complete one siman for worker-slot-13: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot13.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot13.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot13.mjs", [String(siman)]);
...
run("_finalize-siman-slot13.mjs", [String(siman)]);
