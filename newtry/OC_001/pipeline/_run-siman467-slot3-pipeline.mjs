#!/usr/bin/env node
/** Full apply pipeline siman 467 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he467.mjs");
run("_analyze-bad-mt467.mjs");
run("_build-hand467-seed.mjs");
run("_merge-all-hand467-chunks.mjs");
run("_merge-build-fixes-467.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman467-slot3-part${p}.mjs`);
}
