#!/usr/bin/env node
/** Full apply pipeline siman 694 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he694.mjs");
run("_analyze-bad-mt694.mjs");
run("_build-hand694-seed.mjs");
run("_merge-all-hand694-chunks.mjs");
run("_merge-build-fixes-694.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman694-slot3-part${p}.mjs`);
}
