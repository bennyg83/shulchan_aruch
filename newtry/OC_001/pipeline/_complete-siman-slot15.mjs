#!/usr/bin/env node
/** Complete one siman for worker-slot-15: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot15.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot15.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot15.mjs", [String(siman)]);
...
run("_finalize-siman-slot15.mjs", [String(siman)]);
