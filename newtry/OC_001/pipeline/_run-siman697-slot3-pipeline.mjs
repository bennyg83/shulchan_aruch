#!/usr/bin/env node
/** Full apply pipeline siman 697 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he697.mjs");
run("_analyze-bad-mt697.mjs");
run("_build-hand697-seed.mjs");
run("_merge-all-hand697-chunks.mjs");
run("_merge-build-fixes-697.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman697-slot3-part${p}.mjs`);
}
