#!/usr/bin/env node
/** Full apply pipeline siman 466 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he466.mjs");
run("_analyze-bad-mt466.mjs");
run("_build-hand466-seed.mjs");
run("_merge-all-hand466-chunks.mjs");
run("_merge-build-fixes-466.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman466-slot3-part${p}.mjs`);
}
