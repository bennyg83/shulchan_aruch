#!/usr/bin/env node
/** Seed hand-slot18 en from autoFix where preflight+quality pass; report rest */
import fs from "fs";
...
for (const it of hand.items) {
  if (it.en && it.en.length >= 8) {
    const pfKeep = preflightFail(it.en);
    const issuesKeep = runBlockQualityChecks({
      slug: it.slug,
      seif: it.seif,
      marker: it.marker,
      he: it.he,
    ...
    const sevKeep = maxSeverity(issuesKeep);
    if (!pfKeep && sevKeep < SEVERITY.error) {
      ok++;
      continue;
    }
  }
