#!/usr/bin/env node
/** Full apply pipeline siman 684 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he684.mjs");
run("_analyze-bad-mt684.mjs");
run("_build-hand684-seed.mjs");
run("_merge-all-hand684-chunks.mjs");
run("_merge-build-fixes-684.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman684-slot3-part${p}.mjs`);
}
