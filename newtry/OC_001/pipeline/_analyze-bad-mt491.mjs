...
export const PART3 = ["ateret-zekenim","eliyah-rabbah","kaf-hachayyim","rabbi-akiva-eiger","shaarei-teshuvah"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he491-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he491-bad-p${pn}.json`, badKeys[pn].length);
}
