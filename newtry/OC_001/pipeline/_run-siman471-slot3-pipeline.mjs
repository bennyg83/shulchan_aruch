#!/usr/bin/env node
/** Full apply pipeline siman 471 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he471.mjs");
run("_analyze-bad-mt471.mjs");
run("_build-hand471-seed.mjs");
run("_merge-all-hand471-chunks.mjs");
run("_merge-build-fixes-471.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman471-slot3-part${p}.mjs`);
}
