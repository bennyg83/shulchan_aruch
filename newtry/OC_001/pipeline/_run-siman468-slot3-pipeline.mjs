#!/usr/bin/env node
/** Full apply pipeline siman 468 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he468.mjs");
run("_analyze-bad-mt468.mjs");
run("_build-hand468-seed.mjs");
run("_merge-all-hand468-chunks.mjs");
run("_merge-build-fixes-468.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman468-slot3-part${p}.mjs`);
}
