#!/usr/bin/env node
/** Slot3 hand pipeline: 554, 586, 648 — apply hand JSON → bad_mt=0 → progress.log */
import fs from "fs";
...
const SIMANIM = [554, 586, 648];

for (const n of SIMANIM) {
  const pad = String(n).padStart(3, "0");
  const handPath = path.join(__dirname, `_hand-en-${n}.json`);
  if (!fs.existsSync(handPath)) {
    console.error(`Missing ${handPath}`);
    process.exit(1);
  }
  console.log(`\n######## siman ${n} slot3 apply ########`);
  run("_apply-hand-json.mjs", [String(n), handPath]);
  ...
  console.log(`VERIFY siman_${pad}: total=${total} bad_mt=${bad}`);
  if (bad > 0) {
  ...
  console.log(`[COMPLETE] Siman ${n} — ${total} blocks, bad_mt=0, PREFLIGHT none`);
}

console.log("\n[COMPLETE] Session done — simanim: 554, 586, 648");
