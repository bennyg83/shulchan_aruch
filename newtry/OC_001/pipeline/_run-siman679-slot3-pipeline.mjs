#!/usr/bin/env node
/** Full apply pipeline siman 679 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he679.mjs");
run("_analyze-bad-mt679.mjs");
run("_build-hand679-seed.mjs");
run("_merge-all-hand679-chunks.mjs");
run("_merge-build-fixes-679.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman679-slot3-part${p}.mjs`);
}
