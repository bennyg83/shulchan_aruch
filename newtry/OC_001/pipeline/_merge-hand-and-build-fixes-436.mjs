...
function loadHand() {
  const hand = {};
  for (const f of ["siman436-part1.json", "siman436-part2.json"]) {
...
function rel(slug) {
  return `output/siman_436/${slug}/part-001.txt`;
}
...
function buildPart(slugs, partNum, hand) {
  ...
  const outPath = path.join(__dirname, `_fixes-siman436-part${partNum}.mjs`);
  fs.writeFileSync(
    outPath,
    `/** siman 436 part ${partNum} — travel / bedikat chametz */\nexport const fixes = ${JSON.stringify(fixes, null, 2)};\n`
  );
