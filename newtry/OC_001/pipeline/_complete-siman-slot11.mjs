#!/usr/bin/env node
/** Complete one siman for worker-slot-11: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot11.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot11.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot11.mjs", [String(siman)]);
