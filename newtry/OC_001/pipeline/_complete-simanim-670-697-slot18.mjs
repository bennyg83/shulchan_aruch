#!/usr/bin/env node
/** worker-slot-18 — simanim 670-697 (skip already done) */
...
const SKIP = new Set([669, 678, 680, 683]);
const SIMANIM = [];
for (let s = 670; s <= 697; s++) if (!SKIP.has(s)) SIMANIM.push(s);
...
for (const siman of SIMANIM) {
  const pending = collectEditorialBlocks(OUT, siman, "all", "warn", done).length;
  if (!pending) {
  ...
  run("_inject-slot18-translations.mjs", [String(siman)]);
...
console.log("|------:|-------:|:--------:|");
for (const r of rows.sort((a, b) => a.siman - b.siman)) {
...
fs.appendFileSync(
  ...
  `${ts} worker-slot-18 SLOT COMPLETE simanim 669-697\n`,
  "utf8"
);
console.log("\n[COMPLETE] worker-slot-18 simanim 669-697");
