#!/usr/bin/env node
/** Full apply pipeline siman 490 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he490.mjs");
run("_analyze-bad-mt490.mjs");
run("_build-hand490-seed.mjs");
run("_merge-all-hand490-chunks.mjs");
run("_merge-build-fixes-490.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman490-slot3-part${p}.mjs`);
}
