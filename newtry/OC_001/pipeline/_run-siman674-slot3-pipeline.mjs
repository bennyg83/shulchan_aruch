#!/usr/bin/env node
/** Full apply pipeline siman 674 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he674.mjs");
run("_analyze-bad-mt674.mjs");
run("_build-hand674-seed.mjs");
run("_merge-all-hand674-chunks.mjs");
run("_merge-build-fixes-674.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman674-slot3-part${p}.mjs`);
}
