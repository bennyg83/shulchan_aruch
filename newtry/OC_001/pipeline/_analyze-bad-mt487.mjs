...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he487-export.json"), "utf8"));
...
export const PART3 = ["biur-halacha","chatam-sofer","dagul-merevavah","eliyah-rabbah","kaf-hachayyim","rabbi-akiva-eiger","yad-ephraim","chokhmat-shlomo"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he487-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he487-bad-p${pn}.json`, badKeys[pn].length);
}
