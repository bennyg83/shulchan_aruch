#!/usr/bin/env node
/** Full apply pipeline siman 463 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he463.mjs");
run("_analyze-bad-mt463.mjs");
run("_build-hand463-seed.mjs");
run("_merge-all-hand463-chunks.mjs");
run("_merge-build-fixes-463.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman463-slot3-part${p}.mjs`);
}
