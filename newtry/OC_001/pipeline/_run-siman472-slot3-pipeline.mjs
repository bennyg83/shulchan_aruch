#!/usr/bin/env node
/** Full apply pipeline siman 472 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he472.mjs");
run("_analyze-bad-mt472.mjs");
run("_build-hand472-seed.mjs");
run("_merge-all-hand472-chunks.mjs");
run("_merge-build-fixes-472.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman472-slot3-part${p}.mjs`);
}
