#!/usr/bin/env node
/** worker-slot-9 — sprint simanim 362-369 to COMPLETE; closes claim 354-388 */
import fs from "fs";
...
for (const [siman, expected] of SIMANIM) {
  ...
  for (let b = 1; b <= 20; b++) {
    for (const bf of [
      path.join(__dirname, `_fixes-siman${siman}-batch${String.fromCharCode(64 + b)}-slot9.mjs`),
      path.join(__dirname, `_fixes-siman${siman}-batch${b}-slot9.mjs`),
    ]) {
      if (fs.existsSync(bf)) run("_inject-hand-en-slot9.mjs", [String(siman), bf]);
    }
  }
  ...
  run("_finalize-siman-slot9.mjs", [String(siman)]);
