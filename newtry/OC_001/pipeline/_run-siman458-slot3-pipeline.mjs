#!/usr/bin/env node
/** Full apply pipeline siman 458 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he458.mjs");
run("_analyze-bad-mt458.mjs");
run("_build-hand458-seed.mjs");
run("_merge-all-hand458-chunks.mjs");
run("_merge-build-fixes-458.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman458-slot3-part${p}.mjs`);
}
