...
import { partOf } from "./_analyze-bad-mt489.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he489-export.json"), "utf8"));
const handPath = path.join(__dirname, "_hand-en-489.json");
...
for (const pn of [1, 2, 3]) {
  const out = path.join(__dirname, `siman489-part${pn}.json`);
  fs.writeFileSync(out, JSON.stringify(parts[pn], null, 2) + "\n");
  console.log(`siman489-part${pn}.json`, Object.keys(parts[pn]).length);
}
