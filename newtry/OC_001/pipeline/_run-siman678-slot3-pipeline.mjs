#!/usr/bin/env node
/** Full apply pipeline siman 678 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he678.mjs");
run("_analyze-bad-mt678.mjs");
run("_build-hand678-seed.mjs");
run("_merge-all-hand678-chunks.mjs");
run("_merge-build-fixes-678.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman678-slot3-part${p}.mjs`);
}
