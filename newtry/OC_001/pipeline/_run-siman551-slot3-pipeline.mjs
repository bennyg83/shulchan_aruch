#!/usr/bin/env node
/** Full apply pipeline siman 551 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he551.mjs");
run("_analyze-bad-mt551.mjs");
run("_build-hand551-seed.mjs");
run("_merge-all-hand551-chunks.mjs");
run("_merge-build-fixes-551.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman551-slot3-part${p}.mjs`);
}
