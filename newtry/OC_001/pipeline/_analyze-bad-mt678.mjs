...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he678-export.json"), "utf8"));

export const PART1 = ["mechaber","mishnah-berurah","magen-avraham","turei-zahav","beer-hagolah","baer-heitev"];
export const PART2 = ["peri-megadim"];
export const PART3 = ["ateret-zekenim","eliyah-rabbah","kaf-hachayyim","levushei-serad"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he678-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he678-bad-p${pn}.json`, badKeys[pn].length);
}
