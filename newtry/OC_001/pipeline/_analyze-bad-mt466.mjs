...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he466-export.json"), "utf8"));
...
export const PART3 = ["biur-halacha","ateret-zekenim","eliyah-rabbah","eshel-avraham","kaf-hachayyim","rabbi-akiva-eiger","shaarei-teshuvah","chokhmat-shlomo"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he466-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he466-bad-p${pn}.json`, badKeys[pn].length);
}
