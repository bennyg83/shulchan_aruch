#!/usr/bin/env node
/** Merge siman462-part{1,2,3}.json + cite + mechaber → _fixes-siman462-part{1,2,3}.mjs */
import fs from "fs";
...
import { PART1, PART2, PART3 } from "./_analyze-bad-mt462.mjs";
...
async function loadHand() {
  const mech = await import(pathToFileURL(path.join(__dirname, "mech462-en.mjs")).href);
  const hand = {};
  for (const [k, en] of Object.entries(mech.t || {})) hand[`mechaber/${k}`] = en;
  for (const f of ["siman462-part1.json", "siman462-part2.json", "siman462-part3.json"]) {
...
function listPartFiles(slug) {
  const dir = path.join(ROOT, `output/siman_462/${slug}`);
  if (!fs.existsSync(dir)) return [];
  return fs
    .readdirSync(dir)
    .filter((f) => f.endsWith(".txt"))
    .sort()
    .map((f) => `output/siman_462/${slug}/${f}`);
}
...
function buildPart(slugs, partNum, hand) {
  ...
  const outPath = path.join(__dirname, `_fixes-siman462-part${partNum}.mjs`);
  fs.writeFileSync(
    outPath,
    `/** siman 462 part ${partNum} — Pesach */\nexport const fixes = ${JSON.stringify(fixes, null, 2)};\n`
  );
