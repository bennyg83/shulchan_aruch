#!/usr/bin/env node
/** Full apply pipeline siman 677 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he677.mjs");
run("_analyze-bad-mt677.mjs");
run("_build-hand677-seed.mjs");
run("_merge-all-hand677-chunks.mjs");
run("_merge-build-fixes-677.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman677-slot3-part${p}.mjs`);
}
