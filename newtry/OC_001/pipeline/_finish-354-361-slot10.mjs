#!/usr/bin/env node
/** worker-slot-10 — sprint simanim 354-361 to COMPLETE */
import fs from "fs";
...
const startFrom = parseInt(process.argv[2], 10) || 354;
const results = [];
...
for (const [siman, expected] of SIMANIM) {
  ...
  if (!fs.existsSync(path.join(__dirname, `_fixes-siman${siman}-slot10.mjs`))) {
...
fs.appendFileSync(LOG, `${slotTs} worker-slot-10 SLOT COMPLETE simanim 354-361\n`, "utf8");
console.log("\n--- Summary ---");
for (const r of results) {
