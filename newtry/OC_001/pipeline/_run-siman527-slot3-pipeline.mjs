#!/usr/bin/env node
/** Full apply pipeline siman 527 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he527.mjs");
run("_analyze-bad-mt527.mjs");
run("_build-hand527-seed.mjs");
run("_merge-all-hand527-chunks.mjs");
run("_merge-build-fixes-527.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman527-slot3-part${p}.mjs`);
}
