...
const SIMAN = path.join(ROOT, "output/siman_457");
...
for (const slug of fs.readdirSync(SIMAN).sort()) {
  ...
  for (const f of fs.readdirSync(dir).filter((x) => x.endsWith(".txt")).sort()) {
    ...
    for (const b of parseBlocksInFile(raw)) {
      out[`${slug}/${keyFor(b)}`] = { he: b.he, en: b.en, file: `output/siman_457/${slug}/${f}` };
    }
  }
}
const p = path.join(__dirname, "he457-export.json");
