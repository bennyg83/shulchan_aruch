/** Shared worker-slot-18 editorial helpers */
import fs from "fs";
...
export function autoFix(en, marker, he = "") {
  ...
  return scrubEn(t.replace(/\s+/g, " ").trim(), marker);
}

/** Post-MT cleanup: garbage phrases, dot runs, chunk seams, stray Hebrew. */
export function scrubEn(en, marker = "_") {
  let t = String(en ?? "");
  t = t.replace(/\bCapernaum\b/gi, "");
  t = t.replace(/\bthe Lord's Prayer\b/gi, "Hashem");
  t = t.replace(/\bLord's Prayer\b/gi, "Hashem");
  t = t.replace(/[\u0590-\u05FF](?:\s+[\u0590-\u05FF]){1,4}\s*(?=\.|,|\s+[A-Za-z])/g, "");
  t = t.replace(/(?:\s*\.\s*){8,}/g, " ");
  t = t.replace(/\s{2,}/g, " ");
  const plain = plainFromHtml(t);
  const minLen = 55;
  if (plain.length >= minLen * 2 + 20) {
    for (let len = Math.min(90, Math.floor(plain.length / 2)); len >= minLen; len -= 10) {
      for (let i = 0; i <= plain.length - len * 2; i++) {
        const slice = plain.slice(i, i + len);
        if (slice.length < minLen) continue;
        const next = plain.indexOf(slice, i + len);
        if (next !== -1 && next - (i + len) < 12) {
          const a = plain.slice(0, i + len);
          const b = plain.slice(next);
          t = a + b;
          break;
        }
      }
  ...
  const mk = String(marker ?? "_").trim();
  if (/^[א-ת]$/.test(mk) && !t.slice(0, 14).includes(`(${mk})`)) {
    t = `(${mk}) ${t}`;
  }
...
export function writeApplyScript(pipelineDir, siman, batchNum) {
  const applyPath = path.join(pipelineDir, `_apply-siman${siman}-batch${batchNum}-slot18.mjs`);
  const content = `#!/usr/bin/env node
/** worker-slot-18 — siman ${siman} editorial batch ${batchNum} */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { parseBlocksInFile, serializeBlock } from "../oc001_block_lib.mjs";
import { FIXES } from "./_siman${siman}-slot18-batch${batchNum}-data.mjs";
...
export function writeBatchData(pipelineDir, siman, batchNum, batchFixes, count) {
  const outPath = path.join(pipelineDir, `_siman${siman}-slot18-batch${batchNum}-data.mjs`);
  const body = `/** worker-slot-18 — siman ${siman} editorial batch ${batchNum} fixes (${count} blocks) */\nexport const FIXES = ${JSON.stringify(batchFixes, null, 2)};\n`;
  fs.writeFileSync(outPath, body, "utf8");
  return outPath;
}

export function loadHandJson(pipelineDir, siman) {
  const p = path.join(pipelineDir, "work", `hand-slot18-siman-${siman}.json`);
