...
function loadMaster() {
  ...
  if (!fs.existsSync(p)) {
    return {
      ...
      editorial105: { from: 105, to: 697, skipSimanim: [101, 102, 103, 104] },
      publish: { noPublishFromSiman: 105 },
    };
  }
...
export function detectPhase() {
  ...
  if (phase === "oc_complete") return { phase, master: m, activeStats: stats };

  if (phase === "editorial_105_697") {
    const finishThrough = m.finishThrough ?? m.editorial105?.to ?? 697;
    ...
    if (planIdle(stats)) return { phase: "oc_complete", master: m, activeStats: stats };
...
export function advanceIfIdle() {
  ...
  if (phase === "ready_editorial_105" || (phase === "html_presentation" && fs.existsSync(HTML_DONE))) {
    ...
    const to = m.editorial105?.to ?? 697;
    const out = m.editorial105?.planFile || "sprint-plan-105-697.json";
    if (!fs.existsSync(path.join(WORK, out))) {
      runPlan(from, to, out);
    }
    activatePlan(out, "editorial_105_697");
    return { advanced: true, phase: "editorial_105_697", action: `activated ${from}-${to}` };
  }

  if (phase === "editorial_105_697" && planIdle(stats)) {
    const finishThrough = m.finishThrough ?? 697;
    const curTo = stats.range?.to ?? m.editorial105?.to ?? 200;
    if (curTo < finishThrough) {
      ...
      return {
        advanced: true,
        phase: "editorial_105_697",
        action: `next wave simanim ${nextFrom}–${nextTo} (${out})`,
      };
    }
    m.phase = "oc_complete";
    saveMaster(m);
    return { advanced: true, phase: "oc_complete", action: "Orach Chayim editorial complete" };
  }

  if (phase === "ready_editorial_105_wave") {
    const finishThrough = m.finishThrough ?? 697;
    const curTo = m.editorial105?.to ?? 200;
    const waveSize = m.editorial105?.waveSize ?? 96;
    const nextFrom = curTo + 1;
    const nextTo = Math.min(curTo + waveSize, finishThrough);
    const out = `sprint-plan-${nextFrom}-${nextTo}.json`;
