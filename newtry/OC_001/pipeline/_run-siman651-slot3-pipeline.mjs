#!/usr/bin/env node
/** Full apply pipeline siman 651 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he651.mjs");
run("_analyze-bad-mt651.mjs");
run("_build-hand651-seed.mjs");
run("_merge-all-hand651-chunks.mjs");
run("_merge-build-fixes-651.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman651-slot3-part${p}.mjs`);
}
