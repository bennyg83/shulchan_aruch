#!/usr/bin/env node
/** worker-slot-9 — sprint simanim 354-361 to COMPLETE */
import fs from "fs";
...
for (const [siman, expected] of SIMANIM) {
  ...
  if (!fs.existsSync(path.join(__dirname, `_fixes-siman${siman}-slot9.mjs`))) {
...
fs.appendFileSync(LOG, `${slotTs} worker-slot-9 SLOT COMPLETE simanim 354-361\n`, "utf8");
console.log("\n--- Summary ---");
for (const r of results) {
