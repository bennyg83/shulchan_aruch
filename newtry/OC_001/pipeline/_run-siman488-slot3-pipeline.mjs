#!/usr/bin/env node
/** Full apply pipeline siman 488 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he488.mjs");
run("_analyze-bad-mt488.mjs");
run("_build-hand488-seed.mjs");
run("_merge-all-hand488-chunks.mjs");
run("_merge-build-fixes-488.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman488-slot3-part${p}.mjs`);
}
