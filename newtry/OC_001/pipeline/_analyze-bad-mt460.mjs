...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he460-export.json"), "utf8"));
...
export const PART3 = ["biur-halacha","ateret-zekenim","chatam-sofer","eliyah-rabbah","eshel-avraham","kaf-hachayyim","levushei-serad","yad-ephraim","chokhmat-shlomo"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he460-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he460-bad-p${pn}.json`, badKeys[pn].length);
}
