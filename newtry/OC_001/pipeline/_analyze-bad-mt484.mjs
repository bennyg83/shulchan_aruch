...
export const PART1 = ["mechaber","mishnah-berurah","machatzit-hashekel","magen-avraham","turei-zahav","beer-hagolah"];
export const PART2 = ["chok-yaakov","beur-hagra","peri-megadim"];
export const PART3 = ["biur-halacha","eliyah-rabbah","kaf-hachayyim","rabbi-akiva-eiger","yad-ephraim"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he484-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he484-bad-p${pn}.json`, badKeys[pn].length);
}
