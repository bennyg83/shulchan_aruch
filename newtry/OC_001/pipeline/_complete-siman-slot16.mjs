#!/usr/bin/env node
/** Complete one siman for worker-slot-16: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot16.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot16.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot16.mjs", [String(siman)]);
...
run("_finalize-siman-slot16.mjs", [String(siman)]);
