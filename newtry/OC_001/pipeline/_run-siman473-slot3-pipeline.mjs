#!/usr/bin/env node
/** Full apply pipeline siman 473 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he473.mjs");
run("_analyze-bad-mt473.mjs");
run("_build-hand473-seed.mjs");
run("_merge-all-hand473-chunks.mjs");
run("_merge-build-fixes-473.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman473-slot3-part${p}.mjs`);
}
