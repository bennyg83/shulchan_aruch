#!/usr/bin/env node
/**
 * Hands-off editorial pool: coordinator watch (#5) + 4 worker loops until oc_complete.
 *
 *   npm run pipeline:pool:autopilot
 *
 * Requires OC001_POOL_BACKEND=ollama (+ OC001_OLLAMA_URL) or claude-cli.
...
No translation backend for autopilot.
  ...
  Ollama (recommended for hands-off):
    set OC001_POOL_BACKEND=ollama
    set OC001_OLLAMA_URL=http://10.100.102.14:11434
    set OC001_OLLAMA_MODEL_TRANSLATE=qwen2.5:14b-instruct

  Claude CLI (Pro, session limits apply):
    set OC001_POOL_BACKEND=claude-cli
