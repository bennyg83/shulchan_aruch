...
function prefixFromHe(heText, enText) {
  const m = heText.match(/^\(([א-ת"]+)\)/);
  if (!m) return enText;
  const n = gersh[m[1]];
  const rest = enText.replace(/^\(\d+\)\s*/, "");
  return `(${n}) ${rest}`;
}

const enModule = await import(pathToFileURL(path.join(__dirname, "_mb328_en_data.mjs")).href);
const en = enModule.en;

const lines = ["export const t = {"];
for (const k of keys) {
  if (!(k in en)) throw new Error("Missing translation for " + k);
  const val = prefixFromHe(he[k], en[k]).replace(/\\/g, "\\\\").replace(/`/g, "\\`");
  lines.push(`  ${JSON.stringify(k)}: \`${val}\`,`);
}
lines.push("};");
lines.push("");
fs.writeFileSync(path.join(__dirname, "mb328-en.mjs"), lines.join("\n"));
console.log("Wrote mb328-en.mjs with", keys.length, "keys");
