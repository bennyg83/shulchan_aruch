#!/usr/bin/env node
/** Complete one siman for worker-slot-12: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot12.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot12.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot12.mjs", [String(siman)]);
...
run("_finalize-siman-slot12.mjs", [String(siman)]);
