#!/usr/bin/env node
/** Build _fixes-simanNNN-slot14.mjs from pipeline/*NNN-en.mjs slot2 translation maps */
import fs from "fs";
import path from "path";
import { fileURLToPath, pathToFileURL } from "url";
import { parseBlocksInFile } from "../oc001_block_lib.mjs";
import { blockKey } from "./_slot14-lib.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const siman = parseInt(process.argv[2], 10);
if (!siman) throw new Error("Usage: _gen-fixes-siman-slot14-from-en.mjs <siman>");
...
const outPath = path.join(__dirname, `_fixes-siman${siman}-slot14.mjs`);
const count = Object.values(fixes).reduce((n, m) => n + Object.keys(m).length, 0);
const body = `/** worker-slot-14 — siman ${siman} fixes from slot2 en maps (${count} blocks) */\nexport const FIXES = ${JSON.stringify(fixes, null, 2)};\n`;
