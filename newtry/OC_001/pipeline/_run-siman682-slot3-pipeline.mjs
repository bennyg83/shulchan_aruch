#!/usr/bin/env node
/** Full apply pipeline siman 682 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he682.mjs");
run("_analyze-bad-mt682.mjs");
run("_build-hand682-seed.mjs");
run("_merge-all-hand682-chunks.mjs");
run("_merge-build-fixes-682.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman682-slot3-part${p}.mjs`);
}
