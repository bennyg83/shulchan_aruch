#!/usr/bin/env node
/** Full apply pipeline siman 483 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he483.mjs");
run("_analyze-bad-mt483.mjs");
run("_build-hand483-seed.mjs");
run("_merge-all-hand483-chunks.mjs");
run("_merge-build-fixes-483.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman483-slot3-part${p}.mjs`);
}
