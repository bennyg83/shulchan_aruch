#!/usr/bin/env node
/** Full apply pipeline siman 469 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he469.mjs");
run("_analyze-bad-mt469.mjs");
run("_build-hand469-seed.mjs");
run("_merge-all-hand469-chunks.mjs");
run("_merge-build-fixes-469.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman469-slot3-part${p}.mjs`);
}
