#!/usr/bin/env node
/** worker-slot-8 — sprint simanim 343-350 to COMPLETE */
import fs from "fs";
...
const startFrom = parseInt(process.argv[2], 10) || 343;
const results = [];
...
fs.appendFileSync(LOG, `${slotTs} worker-slot-8 SLOT COMPLETE simanim 343-350\n`, "utf8");
console.log("\n--- Summary ---");
for (const r of results) {
