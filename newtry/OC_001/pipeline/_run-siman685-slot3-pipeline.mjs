#!/usr/bin/env node
/** Full apply pipeline siman 685 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he685.mjs");
run("_analyze-bad-mt685.mjs");
run("_build-hand685-seed.mjs");
run("_merge-all-hand685-chunks.mjs");
run("_merge-build-fixes-685.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman685-slot3-part${p}.mjs`);
}
