...
const parts = [1, 2, 3].map((n) =>
  JSON.parse(fs.readFileSync(path.join(__dirname, `siman452-part${n}.json`), "utf8"))
);
...
for (const pn of [1, 2, 3]) {
  ...
  fs.writeFileSync(path.join(__dirname, `he452-queue-p${pn}.json`), JSON.stringify(out, null, 2) + "\n");
  console.log(`he452-queue-p${pn}.json`, Object.keys(out).length);
}
