...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he476-export.json"), "utf8"));
...
export const PART3 = ["eliyah-rabbah","eshel-avraham","kaf-hachayyim","levushei-serad","shaarei-teshuvah","yad-ephraim"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he476-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he476-bad-p${pn}.json`, badKeys[pn].length);
}
