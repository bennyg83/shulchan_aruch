#!/usr/bin/env node
/** Full apply pipeline siman 692 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he692.mjs");
run("_analyze-bad-mt692.mjs");
run("_build-hand692-seed.mjs");
run("_merge-all-hand692-chunks.mjs");
run("_merge-build-fixes-692.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman692-slot3-part${p}.mjs`);
}
