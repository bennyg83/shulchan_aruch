#!/usr/bin/env node
/** Full apply pipeline siman 489 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he489.mjs");
run("_analyze-bad-mt489.mjs");
run("_build-hand489-seed.mjs");
run("_merge-all-hand489-chunks.mjs");
run("_merge-build-fixes-489.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman489-slot3-part${p}.mjs`);
}
