...
import { partOf } from "./_analyze-bad-mt551.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he551-export.json"), "utf8"));
const handPath = path.join(__dirname, "_hand-en-551.json");
...
for (const pn of [1, 2, 3]) {
  const out = path.join(__dirname, `siman551-part${pn}.json`);
  fs.writeFileSync(out, JSON.stringify(parts[pn], null, 2) + "\n");
  console.log(`siman551-part${pn}.json`, Object.keys(parts[pn]).length);
}
