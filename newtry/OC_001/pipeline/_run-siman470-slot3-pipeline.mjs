#!/usr/bin/env node
/** Full apply pipeline siman 470 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he470.mjs");
run("_analyze-bad-mt470.mjs");
run("_build-hand470-seed.mjs");
run("_merge-all-hand470-chunks.mjs");
run("_merge-build-fixes-470.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman470-slot3-part${p}.mjs`);
}
