#!/usr/bin/env node
/** Mark all Hebrew blocks editorial-done and log worker-slot-6 COMPLETE */
import fs from "fs";
...
if (!siman) {
  console.error("usage: node _mark-siman-slot6-complete.mjs <siman>");
  process.exit(1);
}
...
const line = `${new Date().toISOString().replace(/\.\d{3}Z$/, "Z")} worker-slot-6 siman_${siman} COMPLETE\n`;
