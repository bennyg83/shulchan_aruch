#!/usr/bin/env node
/** Merge FIXES { rel: { key: en } } into work/hand-slot10-siman-NNN.json items[].en */
import fs from "fs";
import path from "path";
import { fileURLToPath, pathToFileURL } from "url";
import { autoFix } from "./_slot10-lib.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const siman = parseInt(process.argv[2], 10);
const fixesPath = process.argv[3];
if (!siman || !fixesPath) {
  console.error("Usage: _inject-hand-en-slot10.mjs <siman> <fixes.mjs>");
  process.exit(1);
}
