#!/usr/bin/env node
/** Full apply pipeline siman 484 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he484.mjs");
run("_analyze-bad-mt484.mjs");
run("_build-hand484-seed.mjs");
run("_merge-all-hand484-chunks.mjs");
run("_merge-build-fixes-484.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman484-slot3-part${p}.mjs`);
}
