#!/usr/bin/env node
/** Full apply pipeline siman 475 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he475.mjs");
run("_analyze-bad-mt475.mjs");
run("_build-hand475-seed.mjs");
run("_merge-all-hand475-chunks.mjs");
run("_merge-build-fixes-475.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman475-slot3-part${p}.mjs`);
}
