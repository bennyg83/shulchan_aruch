#!/usr/bin/env node
/** Complete one siman for worker-slot-14: apply fixes, dictionary, validate, finalize */
...
if (!siman) throw new Error("Usage: _complete-siman-slot14.mjs <siman>");
...
const single = path.join(__dirname, `_fixes-siman${siman}-slot14.mjs`);
if (fs.existsSync(single)) run("_apply-fixes-slot14.mjs", [String(siman)]);
...
run("_finalize-siman-slot14.mjs", [String(siman)]);
