...
import { translateCite434 } from "./lib/translate-cite-434.mjs";
...
function loadHand() {
  const hand = {};
  for (const f of ["siman434-part1.json", "siman434-part2.json"]) {
...
function rel(slug) {
  return `output/siman_434/${slug}/part-001.txt`;
}
...
function buildPart(slugs, partNum, hand) {
  ...
  for (const slug of slugs) {
    ...
    for (const b of blocks) {
      ...
      const hk = `${slug}/${k}`;
      let en = hand[hk];
      if (!en && slug === "beer-hagolah") en = translateCite434(b.he);
      if (!en) missing.push(hk);
      else fixes[file][k] = en;
    }
  }
  const outPath = path.join(__dirname, `_fixes-siman434-part${partNum}.mjs`);
  fs.writeFileSync(
    outPath,
    `/** siman 434 part ${partNum} — bitul chametz */\nexport const fixes = ${JSON.stringify(fixes, null, 2)};\n`
  );
