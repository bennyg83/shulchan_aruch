#!/usr/bin/env node
/** Full apply pipeline siman 477 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he477.mjs");
run("_analyze-bad-mt477.mjs");
run("_build-hand477-seed.mjs");
run("_merge-all-hand477-chunks.mjs");
run("_merge-build-fixes-477.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman477-slot3-part${p}.mjs`);
}
