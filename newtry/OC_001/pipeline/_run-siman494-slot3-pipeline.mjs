#!/usr/bin/env node
/** Full apply pipeline siman 494 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he494.mjs");
run("_analyze-bad-mt494.mjs");
run("_build-hand494-seed.mjs");
run("_merge-all-hand494-chunks.mjs");
run("_merge-build-fixes-494.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman494-slot3-part${p}.mjs`);
}
