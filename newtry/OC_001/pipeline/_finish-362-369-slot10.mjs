#!/usr/bin/env node
/** worker-slot-10 — sprint simanim 362-369 to COMPLETE; closes claim 354-388 */
import fs from "fs";
...
const startFrom = parseInt(process.argv[2], 10) || 362;
const results = [];
...
for (const [siman, expected] of SIMANIM) {
  ...
  run("_gen-fixes-siman-slot10-from-en.mjs", [String(siman)]);
  ...
  // optional per-batch fix modules
  for (let b = 1; b <= 20; b++) {
    for (const bf of [
      path.join(__dirname, `_fixes-siman${siman}-batch${String.fromCharCode(64 + b)}-slot10.mjs`),
      path.join(__dirname, `_fixes-siman${siman}-batch${b}-slot10.mjs`),
    ]) {
      if (fs.existsSync(bf)) run("_inject-hand-en-slot10.mjs", [String(siman), bf]);
    }
  }
