#!/usr/bin/env node
/** Full apply pipeline siman 688 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he688.mjs");
run("_analyze-bad-mt688.mjs");
run("_build-hand688-seed.mjs");
run("_merge-all-hand688-chunks.mjs");
run("_merge-build-fixes-688.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman688-slot3-part${p}.mjs`);
}
