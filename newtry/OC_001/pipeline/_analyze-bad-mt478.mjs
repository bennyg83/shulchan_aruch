...
export const PART3 = ["biur-halacha","eliyah-rabbah","kaf-hachayyim","chokhmat-shlomo"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he478-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he478-bad-p${pn}.json`, badKeys[pn].length);
}
