...
import { FIXES_BY_SIMAN } from "./_manual-fixes-slot10-370-377.mjs";
...
for (const siman of (only ?? Object.keys(FIXES_BY_SIMAN).map(Number))) {
  ...
  const tmp = path.join(__dirname, `_tmp-manual-${siman}-slot10.mjs`);
  ...
  const r = spawnSync(
    process.execPath,
    [path.join(__dirname, "_inject-hand-en-slot10.mjs"), String(siman), tmp],
