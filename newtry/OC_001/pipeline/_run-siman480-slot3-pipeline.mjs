#!/usr/bin/env node
/** Full apply pipeline siman 480 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he480.mjs");
run("_analyze-bad-mt480.mjs");
run("_build-hand480-seed.mjs");
run("_merge-all-hand480-chunks.mjs");
run("_merge-build-fixes-480.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman480-slot3-part${p}.mjs`);
}
