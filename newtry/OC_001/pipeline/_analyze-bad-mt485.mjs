...
const exp = JSON.parse(fs.readFileSync(path.join(__dirname, "he485-export.json"), "utf8"));
...
export const PART3 = ["biur-halacha","eliyah-rabbah","kaf-hachayyim","rabbi-akiva-eiger","shaarei-teshuvah","yad-ephraim"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he485-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he485-bad-p${pn}.json`, badKeys[pn].length);
}
