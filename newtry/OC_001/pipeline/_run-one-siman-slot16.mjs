#!/usr/bin/env node
/** Run full slot16 pipeline for one siman */
...
if (!siman) {
  console.error("usage: node _run-one-siman-slot16.mjs <siman>");
  process.exit(1);
}
...
if (fs.existsSync(needHand)) run("_inject-hand-en-slot16.mjs", [String(siman), needHand]);
for (let b = 1; b <= 40; b++) {
  const hb = path.join(__dirname, `_hand${siman}-b${b}-en.mjs`);
  if (fs.existsSync(hb)) run("_inject-hand-en-slot16.mjs", [String(siman), hb]);
}
run("_force-seed-hand-slot16.mjs", [String(siman)]);
run("_fix-hand-preflight-slot16.mjs", [String(siman)]);
const audit = spawnSync(process.execPath, [path.join(__dirname, "_audit-hand-slot16.mjs"), String(siman)], {
  cwd: OC_ROOT,
  encoding: "utf8",
});
if (audit.status !== 0) {
