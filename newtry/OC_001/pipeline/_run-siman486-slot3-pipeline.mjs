#!/usr/bin/env node
/** Full apply pipeline siman 486 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he486.mjs");
run("_analyze-bad-mt486.mjs");
run("_build-hand486-seed.mjs");
run("_merge-all-hand486-chunks.mjs");
run("_merge-build-fixes-486.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman486-slot3-part${p}.mjs`);
}
