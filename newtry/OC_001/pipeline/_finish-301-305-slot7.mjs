#!/usr/bin/env node
/** worker-slot-7 — sprint simanim 301-305 to COMPLETE */
import fs from "fs";
...
const startFrom = parseInt(process.argv[2], 10) || 301;
const results = [];

for (const [siman, expected] of SIMANIM) {
  if (siman < startFrom) continue;
  console.log(`\n######## siman ${siman} ########`);
  run("_export-he-slot7.mjs", [String(siman)]);
  run("_seed-hand-slot7-partial.mjs", [String(siman)]);
  const manualFixes = path.join(__dirname, `_fixes-siman${siman}-slot7.mjs`);
  if (fs.existsSync(manualFixes)) {
    run("_inject-hand-en-slot7.mjs", [String(siman), manualFixes]);
  }
  if (siman >= 301 && siman <= 305 && fs.existsSync(path.join(__dirname, "_fixes-manual-slot7-301-305.mjs"))) {
    run("_inject-manual-slot7-301-305.mjs", [String(siman)]);
  }
  run("_force-seed-hand-slot7.mjs", [String(siman)]);
  run("_fix-hand-preflight-slot7.mjs", [String(siman)]);
  const audit = spawnSync(
    process.execPath,
    [path.join(__dirname, "_audit-hand-slot7.mjs"), String(siman)],
...
fs.appendFileSync(LOG, `${slotTs} worker-slot-7 SLOT COMPLETE simanim 301-305\n`, "utf8");
console.log("\n--- Summary ---");
