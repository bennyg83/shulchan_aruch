#!/usr/bin/env node
/** Full apply pipeline siman 487 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he487.mjs");
run("_analyze-bad-mt487.mjs");
run("_build-hand487-seed.mjs");
run("_merge-all-hand487-chunks.mjs");
run("_merge-build-fixes-487.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman487-slot3-part${p}.mjs`);
}
