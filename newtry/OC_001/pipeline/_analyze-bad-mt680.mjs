...
export const PART1 = ["mechaber","mishnah-berurah","magen-avraham","beer-hagolah"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he680-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he680-bad-p${pn}.json`, badKeys[pn].length);
}
