#!/usr/bin/env node
/** Run full slot9 pipeline for one siman */
...
if (!siman) {
  console.error("usage: node _run-one-siman-slot9.mjs <siman>");
  process.exit(1);
}
...
if (fs.existsSync(fixes)) run("_inject-hand-en-slot9.mjs", [String(siman), fixes]);
for (let b = 1; b <= 40; b++) {
