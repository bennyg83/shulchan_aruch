#!/usr/bin/env node
/** Full apply pipeline siman 478 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he478.mjs");
run("_analyze-bad-mt478.mjs");
run("_build-hand478-seed.mjs");
run("_merge-all-hand478-chunks.mjs");
run("_merge-build-fixes-478.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman478-slot3-part${p}.mjs`);
}
