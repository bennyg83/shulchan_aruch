/** Shared worker-slot-13 editorial helpers */
import fs from "fs";
...
export function writeApplyScript(pipelineDir, siman, batchNum) {
  const applyPath = path.join(pipelineDir, `_apply-siman${siman}-batch${batchNum}-slot13.mjs`);
  const content = `#!/usr/bin/env node
/** worker-slot-13 — siman ${siman} editorial batch ${batchNum} */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { parseBlocksInFile, serializeBlock } from "../oc001_block_lib.mjs";
import { FIXES } from "./_siman${siman}-slot13-batch${batchNum}-data.mjs";
...
export function writeBatchData(pipelineDir, siman, batchNum, batchFixes, count) {
  const outPath = path.join(pipelineDir, `_siman${siman}-slot13-batch${batchNum}-data.mjs`);
  const body = `/** worker-slot-13 — siman ${siman} editorial batch ${batchNum} fixes (${count} blocks) */\nexport const FIXES = ${JSON.stringify(batchFixes, null, 2)};\n`;
  fs.writeFileSync(outPath, body, "utf8");
  return outPath;
}

export function loadHandJson(pipelineDir, siman) {
  const p = path.join(pipelineDir, "work", `hand-slot13-siman-${siman}.json`);
