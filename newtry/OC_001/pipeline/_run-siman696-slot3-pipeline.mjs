#!/usr/bin/env node
/** Full apply pipeline siman 696 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he696.mjs");
run("_analyze-bad-mt696.mjs");
run("_build-hand696-seed.mjs");
run("_merge-all-hand696-chunks.mjs");
run("_merge-build-fixes-696.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman696-slot3-part${p}.mjs`);
}
