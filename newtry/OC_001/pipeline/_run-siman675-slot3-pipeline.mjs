#!/usr/bin/env node
/** Full apply pipeline siman 675 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he675.mjs");
run("_analyze-bad-mt675.mjs");
run("_build-hand675-seed.mjs");
run("_merge-all-hand675-chunks.mjs");
run("_merge-build-fixes-675.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman675-slot3-part${p}.mjs`);
}
