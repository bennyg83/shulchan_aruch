#!/usr/bin/env node
/** Full apply pipeline siman 498 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he498.mjs");
run("_analyze-bad-mt498.mjs");
run("_build-hand498-seed.mjs");
run("_merge-all-hand498-chunks.mjs");
run("_merge-build-fixes-498.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman498-slot3-part${p}.mjs`);
}
