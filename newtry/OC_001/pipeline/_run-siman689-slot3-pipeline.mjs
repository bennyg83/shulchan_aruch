#!/usr/bin/env node
/** Full apply pipeline siman 689 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he689.mjs");
run("_analyze-bad-mt689.mjs");
run("_build-hand689-seed.mjs");
run("_merge-all-hand689-chunks.mjs");
run("_merge-build-fixes-689.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman689-slot3-part${p}.mjs`);
}
