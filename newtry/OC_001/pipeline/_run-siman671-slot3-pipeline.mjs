#!/usr/bin/env node
/** Full apply pipeline siman 671 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he671.mjs");
run("_analyze-bad-mt671.mjs");
run("_build-hand671-seed.mjs");
run("_merge-all-hand671-chunks.mjs");
run("_merge-build-fixes-671.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman671-slot3-part${p}.mjs`);
}
