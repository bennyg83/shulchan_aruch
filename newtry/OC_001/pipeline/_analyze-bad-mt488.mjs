...
export const PART3 = ["eliyah-rabbah","kaf-hachayyim","shaarei-teshuvah"];
...
for (const pn of [1, 2, 3]) {
  fs.writeFileSync(
    path.join(__dirname, `he488-bad-p${pn}.json`),
    JSON.stringify(badKeys[pn], null, 2) + "\n"
  );
  console.log(`he488-bad-p${pn}.json`, badKeys[pn].length);
}
