#!/usr/bin/env node
/** Full apply pipeline siman 495 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he495.mjs");
run("_analyze-bad-mt495.mjs");
run("_build-hand495-seed.mjs");
run("_merge-all-hand495-chunks.mjs");
run("_merge-build-fixes-495.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman495-slot3-part${p}.mjs`);
}
