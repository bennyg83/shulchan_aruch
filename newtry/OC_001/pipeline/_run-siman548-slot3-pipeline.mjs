#!/usr/bin/env node
/** Full apply pipeline siman 548 (452 pattern): export → seed → hand merge → build fixes → apply 3 parts → bad_mt=0 → progress.log */
...
run("_export-he548.mjs");
run("_analyze-bad-mt548.mjs");
run("_build-hand548-seed.mjs");
run("_merge-all-hand548-chunks.mjs");
run("_merge-build-fixes-548.mjs");

for (let p = 1; p <= 3; p++) {
  run(`_apply-siman548-slot3-part${p}.mjs`);
}
