#!/usr/bin/env node
/** Merge hand494-p{N}-*.mjs chunks into siman494-part{N}.json */
import fs from "fs";
...
async function mergePart(p) {
  const hand = JSON.parse(fs.readFileSync(path.join(__dirname, `siman494-part${p}.json`), "utf8"));
  const files = fs.readdirSync(__dirname).filter((f) => f.match(new RegExp(`^hand494-p${p}-.+\\.mjs$`)));
  let n = 0;
  ...
  fs.writeFileSync(path.join(__dirname, `siman494-part${p}.json`), JSON.stringify(hand, null, 2) + "\n");
