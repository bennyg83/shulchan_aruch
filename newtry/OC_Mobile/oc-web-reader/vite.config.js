...
function fileRevision(filePath) {
  // Content hash so Workbox invalidates precache when corpus bundles change.
  ...
  for (const vol of volumes) {
    ...
    try {
      if (fs.existsSync(catalogPath)) {
        entries.push({
          url: `${base}corpus/${vol}/catalog.json`,
          revision: fileRevision(catalogPath),
        });
      }
      ...
      entries.push(
        ...files.map((f) => {
          const full = path.join(bundlesDir, f);
          return {
            url: `${base}corpus/${vol}/bundles/${f}`,
            revision: fileRevision(full),
          };
        })
      );
    } catch {
